...
proc NSColorPreferences::DisplayCmd {oop message first} {
...
proc NSColorPreferences::SetHook {oop hook} {
	...
	eval $win.frameCanvas.yscroll set [$canvas yview]
...
proc NSColorPreferences::ColorChanged {oop pickerId index} {
	...
	CallHook $oop set_color $item \
		[GetValueFromTag part : $tagList] [palette set $index]
...
proc NSColorPreferences::Add_Bevel {canvas x1 y1 x2 y2 light dark width} {
	...
	lappend itemId [Add_Line $canvas $dark $width $x1 $y2 $x2 $y2 $x2 $y1]
	return $itemId
}
