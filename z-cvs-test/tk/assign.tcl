...
proc NSAssign::InitModule {} {
	...
	set Priv(hook) {}
	lappend Priv(hook) hook_character [mc Character]
				...
				sprite {
					...
					foreach row [NSCanvist::Selection $canvistId] {
						if {!$row && [Global autoAssign]} {
							...
							if {[string equal $answer no]} continue
							Global autoAssign 0
						}
						assign set character $row $assign
						if {!$row} {
							UpdateList $oop group 0 icon -assign $assign
						}
		...
		menu_cmd {
			switch -- [lindex $args 0] {
				auto_assign {
					...
					set answer [tk_messageBox -title "Character Icon" \
						-message $msg -type yesno -parent [Info $oop win]]
					...
					if {[NSCanvist::Info [Info $oop member,canvistId] count]} {
						UpdateList $oop member 0 icon -assign [angband player icon]
					}
					UpdateList $oop group 0 icon -assign [angband player icon]
				}
			}
		}
