...
int next_to_walls(int y, int x)
{
	...
	if (cave_floor_bold(y + 1, x))
		k++;
	if (cave_floor_bold(y - 1, x))
		k++;
	if (cave_floor_bold(y, x + 1))
		k++;
	if (cave_floor_bold(y, x - 1))
		k++;
...
void build_tunnel(int row1, int col1, int row2, int col2)
{
	...
	while ((row1 != row2) || (col1 != col2))
	{
		...
		if (c_ptr->feat == FEAT_WALL_OUTER)
		{
			...
			if (cave[y][x].feat == FEAT_PERM_SOLID)
				continue;
			if (cave[y][x].feat == FEAT_PERM_OUTER)
				continue;
...
static bool set_tunnel(int *x, int *y, bool affectwall)
{
	...
	if ((feat == FEAT_PERM_OUTER) || (feat == FEAT_PERM_INNER) ||
 ...
 * the surface.
 */
static void short_seg_hack(int x1, int y1, int x2, int y2, int type,
	int count, bool * fail)
{
	int i, x, y;
	int length;
		...
		for (i = 0; i <= length; i++)
		{
			...
			if (!set_tunnel(&x, &y, TRUE))
			{
				...
				short_seg_hack(x, y, x1 + (i - 1) * (x2 - x1) / length,
					y1 + (i - 1) * (y2 - y1) / length, 1, count, fail);
				short_seg_hack(x, y, x1 + (i + 1) * (x2 - x1) / length,
					y1 + (i + 1) * (y2 - y1) / length, 1, count, fail);
			}
		}
...
bool build_tunnel2(int x1, int y1, int x2, int y2, int type, int cutoff)
{
	...
	if (length > cutoff)
	{
		...
		if (midval == FEAT_WALL_SOLID)
		{
			...
			dx = 0;
			while ((i > 0) &&
				(cave[y3 + dy][x3 + dx].feat == FEAT_WALL_SOLID))
			{
				dy = rand_int(3) - 1;
				dx = rand_int(3) - 1;
