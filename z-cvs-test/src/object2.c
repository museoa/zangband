			...
			this_o_idx = next_o_idx)
		{
			...
			if (this_o_idx == o_idx)
			{
				...
				if (prev_o_idx == 0)
				{
					...
					if (g_grid[y][x].o_idx)
						g_grid[y][x].o_idx = next_o_idx; /* TNB */
				}
...
void compact_objects(int size)
{
	...
	for (num = 0, cnt = 1; num < size; cnt++)
	{
		...
		for (i = 1; i < o_max; i++)
		{
			...
			if ((artifact_p(o_ptr) || o_ptr->art_name) && (cnt < 1000))
				chance = 100;
...
s32b flag_cost(object_type * o_ptr, int plusses)
{
	...
	if (o_ptr->art_name && (o_ptr->art_flags3 & TR3_ACTIVATE))
	{
		...
		if (type == ACT_SUNLIGHT)
			total += 250;
		else if (type == ACT_BO_MISS_1)
			total += 250;
		else if (type == ACT_BA_POIS_1)
			total += 300;
		else if (type == ACT_BO_ELEC_1)
			total += 250;
		else if (type == ACT_BO_ACID_1)
			total += 250;
		else if (type == ACT_BO_COLD_1)
			total += 250;
		else if (type == ACT_BO_FIRE_1)
			total += 250;
		else if (type == ACT_BA_COLD_1)
			total += 750;
		else if (type == ACT_BA_FIRE_1)
			total += 1000;
		else if (type == ACT_DRAIN_1)
			total += 500;
		else if (type == ACT_BA_COLD_2)
			total += 1250;
		else if (type == ACT_BA_ELEC_2)
			total += 1500;
		else if (type == ACT_DRAIN_2)
			total += 750;
		else if (type == ACT_VAMPIRE_1)
			total = 1000;
		else if (type == ACT_BO_MISS_2)
			total += 1000;
		else if (type == ACT_BA_FIRE_2)
			total += 1750;
		else if (type == ACT_BA_COLD_3)
			total += 2500;
		else if (type == ACT_BA_ELEC_3)
			total += 2500;
		else if (type == ACT_WHIRLWIND)
			total += 7500;
		else if (type == ACT_VAMPIRE_2)
			total += 2500;
		else if (type == ACT_CALL_CHAOS)
			total += 5000;
		else if (type == ACT_ROCKET)
			total += 5000;
		else if (type == ACT_DISP_EVIL)
			total += 4000;
		else if (type == ACT_DISP_GOOD)
			total += 3500;
		else if (type == ACT_BA_MISS_3)
			total += 5000;
		else if (type == ACT_CONFUSE)
			total += 500;
		else if (type == ACT_SLEEP)
			total += 750;
		else if (type == ACT_QUAKE)
			total += 600;
		else if (type == ACT_TERROR)
			total += 2500;
		else if (type == ACT_TELE_AWAY)
			total += 2000;
		else if (type == ACT_GENOCIDE)
			total += 10000;
		else if (type == ACT_MASS_GENO)
			total += 10000;
		else if (type == ACT_CHARM_ANIMAL)
			total += 7500;
		else if (type == ACT_CHARM_UNDEAD)
			total += 10000;
		else if (type == ACT_CHARM_OTHER)
			total += 10000;
		else if (type == ACT_CHARM_ANIMALS)
			total += 12500;
		else if (type == ACT_CHARM_OTHERS)
			total += 17500;
		else if (type == ACT_SUMMON_ANIMAL)
			total += 10000;
		else if (type == ACT_SUMMON_PHANTOM)
			total += 12000;
		else if (type == ACT_SUMMON_ELEMENTAL)
			total += 15000;
		else if (type == ACT_SUMMON_DEMON)
			total += 20000;
		else if (type == ACT_SUMMON_UNDEAD)
			total += 20000;
		else if (type == ACT_CURE_LW)
			total += 500;
		else if (type == ACT_CURE_MW)
			total += 750;
		else if (type == ACT_REST_LIFE)
			total += 7500;
		else if (type == ACT_REST_ALL)
			total += 15000;
		else if (type == ACT_CURE_700)
			total += 10000;
		else if (type == ACT_CURE_1000)
			total += 15000;
		else if (type == ACT_ESP)
			total += 1500;
		else if (type == ACT_BERSERK)
			total += 800;
		else if (type == ACT_PROT_EVIL)
			total += 5000;
		else if (type == ACT_RESIST_ALL)
			total += 5000;
		else if (type == ACT_SPEED)
			total += 15000;
		else if (type == ACT_XTRA_SPEED)
			total += 25000;
		else if (type == ACT_WRAITH)
			total += 25000;
		else if (type == ACT_INVULN)
			total += 25000;
		else if (type == ACT_LIGHT)
			total += 150;
		else if (type == ACT_MAP_LIGHT)
			total += 500;
		else if (type == ACT_DETECT_ALL)
			total += 1000;
		else if (type == ACT_DETECT_XTRA)
			total += 12500;
		else if (type == ACT_ID_FULL)
			total += 10000;
		else if (type == ACT_ID_PLAIN)
			total += 1250;
		else if (type == ACT_RUNE_EXPLO)
			total += 4000;
		else if (type == ACT_RUNE_PROT)
			total += 10000;
		else if (type == ACT_SATIATE)
			total += 2000;
		else if (type == ACT_DEST_DOOR)
			total += 100;
		else if (type == ACT_STONE_MUD)
			total += 1000;
		else if (type == ACT_RECHARGE)
			total += 1000;
		else if (type == ACT_ALCHEMY)
			total += 10000;
		else if (type == ACT_DIM_DOOR)
			total += 10000;
		else if (type == ACT_TELEPORT)
			total += 2000;
		else if (type == ACT_RECALL)
			total += 7500;
	}
...
s32b object_value_real(object_type * o_ptr)
{
	...
	switch (o_ptr->tval)
	{
		...
		case TV_WAND:
		{
			 ...
			 */
			value +=
				(value * o_ptr->pval / o_ptr->number / (k_ptr->pval * 2));
...
#if 1 /* Could turn into a BUG -- TNB */
		...
		case TV_SOFT_ARMOR:
		{
			...
			if (power > 1)
			{
				...
				if ((o_ptr->tval == TV_SOFT_ARMOR) &&
					(o_ptr->sval == SV_ROBE) && (rand_int(100) < 10))
				{
	...
	switch (o_ptr->tval)
	{
		...
		case TV_FIGURINE:
		{
			...
			while (1)
			{
				...
				check =
					(dun_level <
					r_ptr->level) ? (r_ptr->level - dun_level) : 0;
		...
		case TV_CORPSE:
		{
			...
			if (cheat_peek)
			{
				msg_format("Corpse of %s, depth +%d", r_name + r_ptr->name,
					check - 1);
			}
	...
	bool great, bool curse)
{
	...
	switch (o_ptr->tval)
	{
		...
		{
#if 1
			if (power || ((o_ptr->tval == TV_HELM) &&
					(o_ptr->sval == SV_DRAGON_HELM)) ||
				((o_ptr->tval == TV_SHIELD) &&
					(o_ptr->sval == SV_DRAGON_SHIELD)) ||
				((o_ptr->tval == TV_CLOAK) &&
					(o_ptr->sval == SV_ELVEN_CLOAK)))
	...
	switch (k_ptr->tval)
	{
		...
		case TV_AMULET:
		{
			if (k_ptr->sval == SV_AMULET_THE_MAGI)
				return (TRUE);
			if (k_ptr->sval == SV_AMULET_RESISTANCE)
				return (TRUE);
			return (FALSE);
		}
	}
...
bool make_object(object_type * j_ptr, bool good, bool great)
{
	...
	if (!cursed_p(j_ptr) && !broken_p(j_ptr) && (obj_level > dun_level))
	{
		/* Rating increase */
...
s16b drop_near(object_type * j_ptr, int chance, int y, int x)
{
	...
	if (!(j_ptr->art_name || artifact_p(j_ptr)) &&
		(rand_int(100) < chance))
	{
		/* Message */
	...
	for (dy = -3; dy <= 3; dy++)
	{
		...
		for (dx = -3; dx <= 3; dx++)
		{
			...
			if ((c_ptr->feat != FEAT_FLOOR) &&
				...
				(c_ptr->feat != FEAT_GRASS) && (c_ptr->feat != FEAT_DIRT)
	...
	for (i = 0; !flag; i++)
	{
		...
		if ((c_ptr->feat != FEAT_FLOOR) && (c_ptr->feat != FEAT_SHAL_WATER)
			&& (c_ptr->feat != FEAT_GRASS) && (c_ptr->feat != FEAT_DIRT) &&
			(c_ptr->feat != FEAT_SHAL_LAVA))
			continue;
	...
	if (!(artifact_p(j_ptr) || j_ptr->art_name))
	{
		...
		if ((c_ptr->feat == FEAT_SHAL_LAVA) && ((j_ptr->tval == TV_STAFF)
				|| (j_ptr->tval == TV_SCROLL) || (j_ptr->tval == TV_WAND)))
		{
			...
			{
				/* Message */
				msg_format("The %s%s burns in the lava.", o_name,
					(plural ? "" : "s"));
			}

			/* Debug */
			if (wizard)
				msg_print("(contact with lava)");
		...
		if ((c_ptr->feat == FEAT_SHAL_WATER) && (j_ptr->tval == TV_RING))
		{
			...
			{
				/* Message */
				msg_format("The %s disappear%s.", o_name,
					(plural ? "" : "s"));
			}

			/* Debug */
			if (wizard)
				msg_print("(contact with water)");
...
void reorder_pack(void)
{
	...
	for (i = 0; i < INVEN_PACK; i++)
	{
		...
		if ((i == INVEN_PACK) && (inven_cnt == INVEN_PACK))
			break;
