	...
	u32b * f6p)
{
	...
	{
		f5 &=
			~(RF5_BO_COLD | RF5_BO_FIRE | RF5_BO_ACID | RF5_BO_ELEC |
...
bool clean_shot(int y1, int x1, int y2, int x2, bool friend)
{
	...
	{
		grid_n =
			project_path(grid_g, MAX_RANGE, y1, x1, y2, x2, PROJECT_FRND);
	}
	else
	{
		grid_n = project_path(grid_g, MAX_RANGE, y1, x1, y2, x2, 0);
	...
	/* May not end in an unrequested grid */
	if ((grid_g[grid_n - 1].y != y2) || (grid_g[grid_n - 1].x != x2))
		return (FALSE);
...
void curse_equipment(int chance, int heavy_chance)
{
	...
	if ((randint(100) <= heavy_chance) && (o_ptr->name1 || o_ptr->name2 ||
			o_ptr->art_name))
	{
...
static int choose_attack_spell(int m_idx, u32b f4, u32b f5, u32b f6)
{
	...
	if (!stupid_monsters && !(r_ptr->flags2 & (RF2_STUPID)))
	{
		...
		else if (has_invul && !(m_ptr->invulner) && (rand_int(100) < 50))
		{
		...
		/* Haste self if we aren't already somewhat hasted (rarely) */
			...
			(rand_int(100) < (20 + r_ptr->speed - m_ptr->mspeed)))
		{
			/* Choose haste spell */
...
bool make_attack_spell(int m_idx)
{
	...
	if (!stupid_monsters)
	{
		...
		if (((f4 & RF4_BOLT_MASK) || (f5 & RF5_BOLT_MASK) ||
				(f6 & RF6_BOLT_MASK)) && !(r_ptr->flags2 & RF2_STUPID) &&
		...
		if (((f4 & RF4_SUMMON_MASK) || (f5 & RF5_SUMMON_MASK) ||
				(f6 & RF6_SUMMON_MASK)) && !(r_ptr->flags2 & RF2_STUPID) &&
	...
	switch (thrown_spell)
	{
		...
		case 128 + 0:
		{
			...
			breath(m_idx, GF_ACID, randint(rlev * 3) + 15, 2, FALSE);
			update_smart_learn(m_idx, DRS_ACID);
			break;
		}
		...
		case 128 + 1:
		{
			...
			breath(m_idx, GF_ELEC, randint(rlev * 3 / 2) + 8, 2, FALSE);
			update_smart_learn(m_idx, DRS_ELEC);
			break;
		}
		...
		case 128 + 2:
		{
			...
			breath(m_idx, GF_FIRE, randint(rlev * 7 / 2) + 10, 2, FALSE);
			update_smart_learn(m_idx, DRS_FIRE);
			break;
		}
		...
		case 128 + 3:
		{
			...
			breath(m_idx, GF_COLD, randint(rlev * 3 / 2) + 10, 2, FALSE);
			update_smart_learn(m_idx, DRS_COLD);
			break;
		}
		...
		case 128 + 5:
		{
			...
			breath(m_idx, GF_NETHER, (50 + damroll(10, 10) + rlev), 2,
				FALSE);
			update_smart_learn(m_idx, DRS_NETH);
			break;
		}
		...
		case 128 + 6:
		{
			...
			breath(m_idx, GF_WATER, randint(rlev * 5 / 2) + 50, 4, FALSE);
			break;
		}
		...
		case 160 + 16:
		{
			...
			{
				count +=
					summon_specific(m_idx, y, x, rlev, SUMMON_KIN, TRUE,
					FALSE, FALSE);
			}
			if (blind && count)
		...
		case 160 + 17:
		{
			...
			if (blind && count)
				msg_print("You hear heavy steps nearby.");
		...
		case 160 + 18:
		{
			...
			{
				count +=
					summon_specific(m_idx, y, x, rlev, 0, TRUE, FALSE,
					FALSE);
			}
			if (blind && count)
		...
		case 160 + 20:
		{
			...
			{
				count +=
					summon_specific(m_idx, y, x, rlev, SUMMON_ANT, TRUE,
					FALSE, FALSE);
			}
			if (blind && count)
				msg_print("You hear many things appear nearby.");
		...
		case 160 + 21:
		{
			...
			{
				count +=
					summon_specific(m_idx, y, x, rlev, SUMMON_SPIDER, TRUE,
					FALSE, FALSE);
			}
			if (blind && count)
				msg_print("You hear many things appear nearby.");
		...
		case 160 + 22:
		{
			...
			{
				count +=
					summon_specific(m_idx, y, x, rlev, SUMMON_HOUND, TRUE,
					FALSE, FALSE);
			}
			if (blind && count)
				msg_print("You hear many things appear nearby.");
		...
		case 160 + 23:
		{
			...
			{
				count +=
					summon_specific(m_idx, y, x, rlev, SUMMON_HYDRA, TRUE,
					FALSE, FALSE);
			}
			if (blind && count)
				msg_print("You hear many things appear nearby.");
		...
		case 160 + 24:
		{
			...
			{
				count +=
					summon_specific(m_idx, y, x, rlev, SUMMON_ANGEL, TRUE,
					FALSE, FALSE);
			}
			if (blind && count)
				msg_print("You hear something appear nearby.");
		...
		case 160 + 25:
		{
			...
			{
				count +=
					summon_specific(m_idx, y, x, rlev, SUMMON_DEMON, TRUE,
					FALSE, FALSE);
			}
			if (blind && count)
				msg_print("You hear something appear nearby.");
		...
		case 160 + 26:
		{
			...
			{
				count +=
					summon_specific(m_idx, y, x, rlev, SUMMON_UNDEAD, TRUE,
					FALSE, FALSE);
			}
			if (blind && count)
				msg_print("You hear something appear nearby.");
		...
		case 160 + 27:
		{
			...
			{
				count +=
					summon_specific(m_idx, y, x, rlev, SUMMON_DRAGON, TRUE,
					FALSE, FALSE);
			}
			if (blind && count)
				msg_print("You hear something appear nearby.");
		...
		case 160 + 28:
		{
			...
			{
				count +=
					summon_specific(m_idx, y, x, rlev, SUMMON_HI_UNDEAD,
					TRUE, FALSE, FALSE);
			}
			if (blind && count)
			{
		...
		case 160 + 29:
		{
			...
			{
				count +=
					summon_specific(m_idx, y, x, rlev, SUMMON_HI_DRAGON,
					TRUE, FALSE, FALSE);
			}
			if (blind && count)
			{
		...
		case 160 + 30:
		{
			...
			{
				count +=
					summon_specific(m_idx, y, x, rlev, SUMMON_AMBERITES,
					TRUE, FALSE, FALSE);
			}
			if (blind && count)
			{
		...
		case 160 + 31:
		{
			...
			{
				count +=
					summon_specific(m_idx, y, x, rlev, SUMMON_UNIQUE, TRUE,
					FALSE, FALSE);
			}
			for (k = 0; k < 8; k++)
			{
