...
typedef struct
{
	...
	cptr misc_magic[N_ELEMENTS(misc_flags2_desc) +
		N_ELEMENTS(misc_flags3_desc) + 1 /* Permanent Light */  + 1	/* type of curse */
		 + 1]; /* sentinel NULL */
...
static cptr *spoiler_flag_aux(const u32b art_flags,
	const flag_desc * flag_ptr, cptr * desc_ptr, const int n_elmnts)
{
	int i;
...
static void analyze_pval(object_type * o_ptr, pval_info_type * p_ptr)
{
	...
	{
		affects_list =
			spoiler_flag_aux(f1, stat_flags_desc, affects_list,
			N_ELEMENTS(stat_flags_desc));
	}

	/* And now the "rest" */
	affects_list =
		spoiler_flag_aux(f1, pval_flags1_desc, affects_list,
...
static void analyze_slay(object_type * o_ptr, cptr * slay_list)
{
	...
	slay_list =
		spoiler_flag_aux(f1, slay_flags_desc, slay_list,
...
static void analyze_brand(object_type * o_ptr, cptr * brand_list)
{
	...
	brand_list =
		spoiler_flag_aux(f1, brand_flags_desc, brand_list,
...
static void analyze_resist(object_type * o_ptr, cptr * resist_list)
{
	...
	resist_list =
		spoiler_flag_aux(f2, resist_flags_desc, resist_list,
		N_ELEMENTS(resist_flags_desc));
...
static void analyze_immune(object_type * o_ptr, cptr * immune_list)
{
	...
	immune_list =
		spoiler_flag_aux(f2, immune_flags_desc, immune_list,
		N_ELEMENTS(immune_flags_desc));
...
static void analyze_sustains(object_type * o_ptr, cptr * sustain_list)
{
	...
	{
		sustain_list =
			spoiler_flag_aux(f2, sustain_flags_desc, sustain_list,
...
static void analyze_misc_magic(object_type * o_ptr, cptr * misc_list)
{
	...
	misc_list =
		spoiler_flag_aux(f2, misc_flags2_desc, misc_list,
		N_ELEMENTS(misc_flags2_desc));

	misc_list =
		spoiler_flag_aux(f3, misc_flags3_desc, misc_list,
...
static void spoil_mon_info(cptr fname)
{
	...
	for (n = 0; n < count; n++)
	{
		...
		{
			sprintf(buf, "%s is never found out of depth.  ",
				wd_che[msex]);
			spoil_out(buf);
		}
		...
		{
			sprintf(buf, "%s is surrounded by electricity.  ",
				wd_che[msex]);
			spoil_out(buf);
		}
		...
		if (flags5 & (RF5_BA_MANA))
			vp[vn++] = "produce mana storms";
		if (flags5 & (RF5_BA_DARK))
			vp[vn++] = "produce darkness storms";
		...
		i = 0;
		if (flags1 & (RF1_DROP_60))
			i += 1;
		if (flags1 & (RF1_DROP_90))
			i += 2;
		if (flags1 & (RF1_DROP_1D2))
			i += 2;
		if (flags1 & (RF1_DROP_2D2))
			i += 4;
		if (flags1 & (RF1_DROP_3D2))
			i += 6;
		if (flags1 & (RF1_DROP_4D2))
			i += 8;
			...
			if (r_ptr->blow[j].effect)
			{
				...
				if (r_ptr->blow[j].d_dice && r_ptr->blow[j].d_side)
				{
					...
					if (r_ptr->blow[j].d_side == 1)
						...
						sprintf(buf, " %dd%d", r_ptr->blow[j].d_dice,
							r_ptr->blow[j].d_side);
					spoil_out(buf);
				}
			}
