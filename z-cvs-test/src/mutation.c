...
bool gain_random_mutation(int choose_mut)
{
	...
	else
	{
		...
		if (p_ptr->prace == RACE_VAMPIRE &&
			!(p_ptr->muta1 & MUT1_HYPN_GAZE) && (randint(10) < 7))
		{
			muta_class = &(p_ptr->muta1);
			muta_which = MUT1_HYPN_GAZE;
			muta_desc = "Your eyes look mesmerizing...";
		...
		else if (p_ptr->prace == RACE_IMP && !(p_ptr->muta2 & MUT2_HORNS)
			&& (randint(10) < 7))
		{
			muta_class = &(p_ptr->muta2);
			muta_which = MUT2_HORNS;
			...
			&& (randint(10) < 7))
		{
			...
			!(p_ptr->muta1 & MUT1_POLYMORPH) && (randint(10) < 2))
		{
			muta_class = &(p_ptr->muta1);
			muta_which = MUT1_POLYMORPH;
			muta_desc = "Your body seems mutable.";
			...
			!(p_ptr->muta2 & MUT2_TENTACLES) && (randint(10) < 7))
		{
			muta_class = &(p_ptr->muta2);
			muta_which = MUT2_TENTACLES;
...
{
	return (count_bits(p_ptr->muta1) + count_bits(p_ptr->muta2) +
	...
	switch (power)
	{
		...
		case MUT1_GROW_MOLD:
			if (racial_aux(1, 6, A_CON, 14))
			{
				...
				for (i = 0; i < 8; i++)
				{
					summon_specific(-1, py, px, lvl, SUMMON_BIZARRE1,
						FALSE, TRUE, TRUE);
				}
			}
			break;
