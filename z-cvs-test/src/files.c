...
errr process_pref_file_aux(char *buf)
{
	...
	else if (buf[0] == 'X')
	{
		for (i = 0; option_info[i].o_desc; i++)
		{
			...
			if (option_info[i].o_var && option_info[i].o_text &&
				streq(option_info[i].o_text, buf + 2))
			{
				/* Clear */
				option_flag[os] &= ~(1L << ob);
				(*option_info[i].o_var) = FALSE;
				Bind_Option(option_info[i].o_text, 0); /* TNB */
				return (0);
			}
		}
	...
	else if (buf[0] == 'Y')
	{
		for (i = 0; option_info[i].o_desc; i++)
		{
				...
				streq(option_info[i].o_text, buf + 2))
			{
				...
				Bind_Option(option_info[i].o_text, 1); /* TNB */
				return (0);
			}
		}
...
errr process_pref_file(cptr name)
{
	...
	if (err)
	{
		...
		angtk_eval("tk_messageBox", "-message",
			format("Error %d in line %d of file '%s'.\nParsing '%s'", err,
				num, name, buf), NULL);
#else /* TNB */
...
{
	angtk_eval("angband_display", "player", "show", NULL);
	(void) inkey();
	angtk_eval("angband_display", "player", "hide", NULL);
}
 ...
 */
static void prt_num(cptr header, s32b num, int row, int col, byte color,
	int wid)
{
...
static void display_player_abilities(void)
{
	...
	if (!muta_att)
		put_str(format("%d", p_ptr->num_blow), 16,
			COL_SKILLS3 + WID_SKILLS);
	else
		put_str(format("%d+%d", p_ptr->num_blow, muta_att), 16,
			COL_SKILLS3 + WID_SKILLS);

	put_str("Shots/Round :", 17, COL_SKILLS3);
	/* Calculate shots (rounded) */
	put_str(format("%d.%d", shots, shot_frac), 17,
		COL_SKILLS3 + WID_SKILLS);
	...
	put_str(format("%d'", p_ptr->see_infra * 10), 19,
		COL_SKILLS3 + WID_SKILLS);
}
 ...
 */
/* static -- TNB */ void player_flags(u32b * f1, u32b * f2, u32b * f3)
{
	/* Clear */
	(*f1) = (*f2) = (*f3) = 0L;
 ...
 */
static void display_player_flag_aux(int row, int col, char *header, int n,
	u32b flag1, u32b flag2)
{
	int i;
	u32b f[3];
...
static void display_player_flag_info(void)
{
	...
	display_player_flag_aux(row++, col, "Acid  :", 2, TR2_RES_ACID,
		TR2_IM_ACID);
	display_player_flag_aux(row++, col, "Elec  :", 2, TR2_RES_ELEC,
		TR2_IM_ELEC);
	display_player_flag_aux(row++, col, "Fire  :", 2, TR2_RES_FIRE,
		TR2_IM_FIRE);
	display_player_flag_aux(row++, col, "Cold  :", 2, TR2_RES_COLD,
		TR2_IM_COLD);
	...
	display_player_flag_aux(row++, col, "SlwDigstn:", 3, TR3_SLOW_DIGEST,
		0);
...
static void display_player_stat_info(void)
{
	...
	for (i = 0; i < 6; i++)
	{
		...
		if ((p_ptr->stat_max[i] <= 18) && (p_ptr->stat_top[i] > 18))
			e_adj =
				(p_ptr->stat_top[i] - 18) / 10 - p_ptr->stat_max[i] + 18;

		if ((p_ptr->stat_max[i] > 18) && (p_ptr->stat_top[i] <= 18))
			e_adj =
				p_ptr->stat_top[i] - (p_ptr->stat_max[i] - 18) / 10 - 19;
...
static void display_player_top(void)
{
	...
	if (p_ptr->realm1)
	{
		c_put_str(TERM_L_BLUE, realm_names[p_ptr->realm1], 6,
			COL_NAME + WID_NAME);
	}

	if (p_ptr->pclass == CLASS_CHAOS_WARRIOR)
	{
		c_put_str(TERM_L_BLUE, chaos_patrons[p_ptr->chaos_patron], 7,
			COL_NAME + WID_NAME);
	}

	else if (p_ptr->realm2)
	{
		c_put_str(TERM_L_BLUE, realm_names[p_ptr->realm2], 7,
			COL_NAME + WID_NAME);
	}
...
static void display_player_middle(void)
{
	...
	prt_num("+ To AC     ", p_ptr->dis_to_a, 11, COL_BONUS, TERM_L_BLUE,
		3);
	...
	prt_num("Level      ", (int) p_ptr->lev, 9, COL_VALUE, TERM_L_GREEN,
		9);
...
bool show_file(cptr name, cptr what, int line, int mode)
{
	...
	angtk_eval("angband_display", "help", "show", name, NULL);
	k = inkey();
...
{
	angtk_eval("angband_display", "help", "show", NULL);
	(void) inkey();
	angtk_eval("angband_display", "help", "hide", NULL);
}
...
bool show_file(cptr name, cptr what, int line, int mode)
{
	...
	while (TRUE)
	{
		...
		prt(format("[ZAngband %d.%d.%d, %s, Line %d/%d]", FAKE_VER_MAJOR,
				FAKE_VER_MINOR, FAKE_VER_PATCH, caption, line, size), 0,
			0);
...
void do_cmd_suicide(void)
{
	...
	else
	{
		...
		if (!noscore)
		{
			...
			prompt_print("Please verify SUICIDE by typing the '@' sign: ");	/* TNB */
			flush();
			i = inkey();
			prt("", 0, 0);
			if (i != '@')
...
long total_points(void)
{
	...
	if (preserve_mode)
		mult -= 10;	/* Penalize preserve, maximize modes */
	if (maximize_mode)
		mult -= 15;
	if (stupid_monsters)
		mult -= 20;	/* AI is not that big a deal (yet) */
	if (vanilla_town)
		mult += 5; /* Vanilla town is harder */
	if (ironman_hard_quests)
		mult += 10;	/* so are hard quests */
	...
	if (mult < 5)
		mult = 5; /* At least 5% of the original score */
...
static void show_info(void)
{
	...
	/* Describe options */
	prt("You may now dump a character record to one or more files.", 21,
		0);
	...
	for (l = 1; l < max_towns; l++)
	{
		...
		if (st_ptr->stock_num)
		{
			...
			for (k = 0, i = 0; i < st_ptr->stock_num; k++)
			{
				...
				/* Caption */
				prt(format("Your home contains (page %d): -more-", k + 1),
					0, 0);
...
void close_game(void)
{
	...
	if (death)
	{
		...
		/* Tomb */
		angtk_eval("angband_display", "tomb", "show", NULL);

		/* Save memories */
		if (death_save && !save_player())
		{
			/* error message */
			angtk_eval("tk_messageBox", "-icon", "error", "-message",
				"Death save failed!", NULL);
		}
		...
		top_twenty();
		angtk_eval("angband_display", "highscore", "show", NULL);
	}

	/* Quit without saving */
	else if (exit_skip_save)
	{
		/* Nothing */
	}
	...
	/* Last call -- TNB */
	angtk_eval("angband_close_game", NULL);	/* TNB */
}
		...
		if (my_fgets(fp, buf, 1024) == 0)
		{
			...
			if ((buf[0] == 'N') && (buf[1] == ':'))
			{
				...
				else
				{
					...
					msg_format("Error in line %d of %s!", line_num,
						file_name);
					my_fclose(fp);
					return (-1);
				}
