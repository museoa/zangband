...
static char *keyword_plane[] = {"feature", "icon1", "icon2", "icon3", "icon4",
	"symbol", "letter", NULL};
...
static int ValidateVault(Tcl_Interp *interp, int vaultId, int *vaultIndex)
{
	...
	Tcl_SetStringObj(resultPtr, format("no such vault \"%d\"", vaultId), -1);
	return TCL_ERROR;
}
...
static int ValidateCoord(Tcl_Interp *interp, int vaultIndex, int y, int x)
{
		...
		(x < vaultPtr->x) || (x >= vaultPtr->x + vaultPtr->width))
	{
		...
		Tcl_SetStringObj(resultPtr,
			format("location \"%d %d\" not in bounds: must be in %d %d %d %d",
			y, x, vaultPtr->y, vaultPtr->x, vaultPtr->y + vaultPtr->height - 1,
			vaultPtr->x + vaultPtr->width - 1), -1);
		return TCL_ERROR;
	}
	return TCL_OK;
}
...
char *IconToString(char *buf, IconSpec *iconSpecPtr)
{
	if (iconSpecPtr->ascii == -1)
	{
		(void) sprintf(buf, "%s %d",
			g_icon_data[iconSpecPtr->type].desc,
			iconSpecPtr->index);
	}
	else
	{
...
int vault_read(Tcl_Interp *interp, int vaultIndex, char *fileName)
{
	...
	if (iconTypeCount >= 50)
	{
		Tcl_SetResult(interp, format("invalid iconTypeCount \"%d\":"
			" must be from 0 to 49", (int) iconTypeCount), TCL_VOLATILE);
		goto error;
	}
		...
		for (y = 0; y < hgt16b; y++)
		{
			for (x = 0; x < wid16b; x++)
			{
				...
				if ((fake[0] == 1) && (fake[1] == 1) && (layer == ICON_LAYER_2))
				{
					...
					if (!g_icon_data[iconSpec2.type].rle_data)
					{
	...
	Tcl_Obj * CONST objv[])
{
	...
	int vaultIndex;
	int y, x, n;
	static char *createSwitch[] = {"-file", "-height", "-width", NULL};
	...
	Tcl_SetIntObj(Tcl_GetObjResult(interp), vaultPtr->id);
	...
	Tcl_Obj * CONST objv[])
{
	...
	if (g_vault_current)
		Tcl_SetIntObj(Tcl_GetObjResult(interp), g_vault_current->id);
	else
	...
	Tcl_Obj * CONST objv[])
{
	...
	VaultDestroyed(interp, vaultIndex);
	for (plane = 0; plane < VLT_PLANE_MAX; plane++)
	...
	Tcl_Obj * CONST objv[])
{
	...
	if (objC == 2)
	{
		Tcl_SetIntObj(Tcl_GetObjResult(interp), vaultPtr->height);
		return TCL_OK;
	}
	...
	Tcl_Obj * CONST objv[])
{
	...
	{
		Tcl_SetResult(interp, "data too tall", TCL_STATIC);
		return TCL_ERROR;
	}
	for (row = 0; row < dataHeight; row++)
	{
		...
		{
			Tcl_SetResult(interp, "data too wide", TCL_STATIC);
			return TCL_ERROR;
		}
	...
	Tcl_Obj * CONST objv[])
{
	...
	if (objC == 3)
	{
		Tcl_SetStringObj(Tcl_GetObjResult(interp),
			map_symbol_name(vaultPtr->symbol[feat]), -1);
		return TCL_OK;
	}
 ...
 * (vault) town ?$v?
 */
int objcmd_vault_town(ClientData clientData, Tcl_Interp * interp, int objc,
	Tcl_Obj * CONST objv[])
{
	CommandInfo *infoCmd = (CommandInfo *) clientData;
	int objC = objc - infoCmd->depth;
	...
	if (objC == 2)
	{
		...
		if (vaultId)
			g_vault_town = g_vault[vaultIndex];
		else
			g_vault_town = NULL;
		return TCL_OK;
	}
	...
	if (g_vault_town)
		Tcl_SetIntObj(Tcl_GetObjResult(interp), g_vault_town->id);
	else
		Tcl_SetIntObj(Tcl_GetObjResult(interp), 0);
	...
	Tcl_Obj * CONST objv[])
{
	...
	if (objC == 2)
	{
		Tcl_SetIntObj(Tcl_GetObjResult(interp), vaultPtr->width);
		return TCL_OK;
	}
	...
	{
		Tcl_SetResult(interp,
			format("invalid width \"%d\": must from %d to %d",
	...
	Tcl_Obj * CONST objv[])
{
	...
	{
		Tcl_AppendResult(interp, "error opening \"", t,
			"\" for writing", NULL);
...
#else
	wr_byte((byte) g_icon_data_count);
	for (i = 0; i < g_icon_data_count; i++)
	{
		char *s = g_icon_data[i].desc;
		(void) fwrite(s, 1, strlen(s) + 1, fp);
...
int Icon_Transparent(t_icon_data *iconDataPtr, int index, int iso)
{
	...
	if (iso)
		return 1;
	return (iconDataPtr->rle_data != NULL);
}
...
void vault_wtd(Widget *widgetPtr, int y, int x, t_display *wtd)
{
	...
	if ((iconFG.type == ICON_TYPE_NONE) && (iconBG.type == ICON_TYPE_NONE))
	{
		...
		FinalIcon(&iconFG, &assign, 1, NULL);
		if (is_sprite(&assign))
			wtd->anim = TRUE;

		/* The foreground icon is transparent */
		if (feat != g_background[feat] /* g_icon_data[iconFG.type].rle_data */)
		{
			/* Get the background feature assignment */
			t_assign assign = g_assign[ASSIGN_FEATURE].assign[g_background[feat]];

			FinalIcon(&iconBG, &assign, 1, NULL);
			if (is_sprite(&assign))
				wtd->anim = TRUE;

			/* Error: background icon is also transparent */
			if (Icon_Transparent(&g_icon_data[iconBG.type], iconBG.index, iso))
			{
	...
	{
		/* The foreground icon is transparent */
		if (Icon_Transparent(&g_icon_data[iconFG.type], iconFG.index, iso))
		{
			/* Error: background icon is also transparent */
			if (Icon_Transparent(&g_icon_data[iconBG.type], iconBG.index, iso))
			{
 ...
 * Called by town_gen().
 */
bool angtk_build_town(void)
{
	int y, x, qy = 0, qx = 0;
	t_vault *vaultPtr;

	g_vault_current = g_vault_town;
	
	/* Apply the vault features, or fail */
	if (!vault_apply_feat(qy, qx)) return (FALSE);

	/* Get the town vault */
	vaultPtr = g_vault_town;
	...
	while (TRUE)
	{
		...
		y = qy + rand_range(3, vaultPtr->height - 4);
		x = qx + rand_range(3, vaultPtr->width - 4);
	...
	/* Remember the size of the town */
	g_cave_hgt = vaultPtr->height;
	g_cave_wid = vaultPtr->width;

	/* Save the size of the town */
	cur_hgt = g_cave_hgt;
	cur_wid = g_cave_wid;

	/* Hack -- Cancel the wilderness mimicry */
	for (x = 0, y = 0; y < cur_hgt; y++)
		cave[y][x].mimic = cave_feat(y, x);
	for (x = 0, y = 0; x < cur_wid; x++)
		cave[y][x].mimic = cave_feat(y, x);
...
/*
 * If a new character was created, or a saved game was restored and
 * the character is on level 0 (the town), we must rebuild the town
 * from scratch. This is because the layout of the town is contained
 * in the tk/config/town file and it may conflict with what was saved
 * in the savefile. The only trick is to preserve the monsters and
 * objects present in the town. This may involve moving any monster or
 * object that is now clobbered by a town feature to a safe location.
 */
void angtk_rebuild_town(void)
{
	int i, y, x, qy, qx;
	 ...
	 */
	stick_to_stair = (cave[py][px].feat == FEAT_MORE);

	qy = 0;
	qx = 0;

	/* Save the size of the town */
	cur_hgt = vaultPtr->height;
...
{
	g_vault = Array_New(1, sizeof(t_vault *));
	g_vault_count = 0;
