...
bool teleport_away(int m_idx, int dis)
{
	...
	while (look)
	{
		...
		for (i = 0; i < 500; i++)
		{
			...
			if (cave[ny][nx].feat == FEAT_GLYPH)
				continue;
			if (cave[ny][nx].feat == FEAT_MINOR_GLYPH)
				continue;
...
void teleport_to_player(int m_idx)
{
	...
	/* "Skill" test */
	if (randint(100) > r_info[m_ptr->r_idx].level)
		return;
...
void teleport_player(int dis)
{
	...
	if (dis > 200)
		dis = 200; /* To be on the safe side... */
	...
	while (xx < 2)
	{
		...
		while (yy < 2)
		{
			...
			else
			{
				...
				{
					if ((r_info[m_list[cave[oy + yy][ox +
										xx].m_idx].r_idx].
							flags6 & RF6_TPORT) &&
						!(r_info[m_list[cave[oy + yy][ox +
										xx].m_idx].r_idx].
							flags3 & RF3_RES_TELE))
						/*
						 ...
						 */
					{
						if (!(m_list[cave[oy + yy][ox + xx].m_idx].csleep))
							teleport_to_player(cave[oy + yy][ox +
									xx].m_idx);
					}
				}
			}
...
void call_the_(void)
{
		...
		cave_floor_bold(py + 1, px) && cave_floor_bold(py + 1, px + 1))
	{
		...
		{
			if (i - 5)
				fire_ball(GF_ROCKET, i, 175, 2);
		}

		for (i = 1; i < 10; i++)
		{
			if (i - 5)
				fire_ball(GF_MANA, i, 175, 3);
		}

		for (i = 1; i < 10; i++)
		{
			if (i - 5)
				fire_ball(GF_NUKE, i, 175, 4);
		}
	}
	else
	{
...
void fetch(int dir, int wgt, bool require_los)
{
	...
	{
		msg_print("You can't fetch to your current location.");
		return;
	}
	...
	if (dir == 5 && target_okay())
	{
		...
		/* Prevent fetching from the character's location */
		if ((ty == py) && (tx == px))
		{
			msg_print("You are standing at the targetted location.");
			return;
		}
		...
		{
#if 1 /* TNB */
			msg_print("There is no object at the targetted location.");
#else /* TNB */
	...
	else
	{
		...
		do
		{
			...
			if (!in_bounds(ty, tx) ||
				(distance(py, px, ty, tx) > MAX_RANGE))
			{
				msg_print("There is no object within range.");
				return;
			}

			/* Require line-of-sight */
			if (require_los && !cave_floor_bold(ty, tx))
			{
				msg_print
					("You have no direct line of sight to that location.");
				return;
	...
	if (1)
	{
		...
		/* Start at the object's location */
		y = ty;
		x = tx;
		...
		while (1)
		{
			/* Stop at the character's location */
			if ((y == py) && (x == px))
				break;

			/* Calculate the new location */
			mmove2(&y, &x, ty, tx, py, px);

			/* Visual effect if the location is visible */
			if (player_can_see_bold(y, x))
			{
				angtk_effect_object(y, x, o_ptr);
				Term_xtra(TERM_XTRA_DELAY, msec);
				angtk_effect_clear(y,x);
			}

			/* The player cannot see the location */
			else
			{
	...
	if (g_grid[ty][tx].o_idx)
		g_grid[ty][tx].o_idx = 0; /* TNB */
...
bool alchemy(void)
{
	...
	else
	{
		...
		if (price > 30000)
			price = 30000;
		msg_format("You turn %s to %ld coins worth of gold.", o_name,
			price);
		p_ptr->au += price;
...
bool enchant(object_type * o_ptr, int n, int eflag)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		if (eflag & ENCH_TOHIT)
		{
			...
			if (force || ((randint(1000) > chance) && (!a ||
						(rand_int(100) < 50))))
			{
				o_ptr->to_h++;
				res = TRUE;
...
bool enchant_spell(int num_hit, int num_dam, int num_ac)
{
	...
	/* Describe */
	msg_format("%s %s glow%s brightly!", ((item >= 0) ? "Your" : "The"),
...
bool artifact_scroll(void)
{
	...
	if (o_ptr->tval == TV_DRAG_ARMOR)
	{
		...
		msg_format("The %s %s already magical!", o_name,
	...
	{
		msg_format("The %s %s already %s!", o_name,
			((o_ptr->number > 1) ? "are" : "is"),
...
{
	if ((p_ptr->muta3 & MUT3_GOOD_LUCK) && !artifact_p(o_ptr) &&
		!o_ptr->art_name && !object_known_p(o_ptr))
	{
		good_luck(o_ptr);
	}
...
bool recharge(int power)
{
	...
	if (fail)
	{
		...
		if (artifact_p(o_ptr))
		{
			object_desc(o_name, o_ptr, TRUE, 0);
			msg_format
				("The recharging backfires - %s is completely drained!",
				o_name);
		...
		else
		{
			...
			{
				if (o_ptr->number > 1)
					msg_format("Wild magic consumes one of your %s!",
						o_name);
				else
...
bool bless_weapon(void)
{
	...
	{
		/* Describe */
		msg_format("%s %s shine%s!", ((item >= 0) ? "Your" : "The"),
			o_name, ((o_ptr->number > 1) ? "" : "s"));
	...
	else
	{
		...
		if ((o_ptr->to_h > 5) && (rand_int(100) < 33))
			o_ptr->to_h--;
		...
		if ((o_ptr->to_d > 5) && (rand_int(100) < 33))
			o_ptr->to_d--;
		...
		if ((o_ptr->to_a > 5) && (rand_int(100) < 33))
			o_ptr->to_a--;
...
void display_spell_list(void)
{
	...
	for (j = 0; j < ((use_realm2 > -1) ? 2 : 1); j++)
	{
		...
		for (i = 0; i < 32; i++)
		{
			...
			/* Access the spell */
			s_ptr =
				&mp_ptr->info[(j < 1) ? use_realm1 : use_realm2][i % 32];
			...
			/* Unknown */
						...
						1) ? (spell_learned1 & (1L << i)) : (spell_learned2
						& (1L << (i % 32)))))
			{
				/* Unknown */
				a = TERM_RED;
			...
			/* Untried */
						...
						1) ? (spell_worked1 & (1L << i)) : (spell_worked2 &
						(1L << (i % 32)))))
			{
				/* Untried */
				a = TERM_YELLOW;
			...
			/* Dump the spell --(-- */
			sprintf(out_val, "%c/%c) %-20.20s", I2A(n / 8), I2A(n % 8),
				name);
...
s16b spell_chance(int spell, int realm)
{
	 ...
	 */
	if ((p_ptr->pclass != CLASS_PRIEST) && (p_ptr->pclass != CLASS_MAGE) &&
...
bool spell_okay(int spell, bool known, int realm)
{
	...
	if ((realm ==
			p_ptr->realm2 -
			1) ? (spell_forgotten2 & (1L << spell)) : (spell_forgotten1 &
			(1L << spell)))
	{
		/* Never okay */
		return (FALSE);
	...
	if ((realm ==
			p_ptr->realm2 -
			1) ? (spell_learned2 & (1L << spell)) : (spell_learned1 & (1L
				<< spell)))
	{
 ...
 */
/* static -- TNB */ void spell_info(char *p, int spell, int realm)
{
	/* Default */
	strcpy(p, "");
		...
		switch (realm)
		{
			...
			case 3: /* Chaos */
				switch (spell)
				{
					...
					case 4:
						sprintf(p, " dam 3d5+%d",
							plev + (plev / (((p_ptr->pclass == CLASS_MAGE)
										|| (p_ptr->pclass ==
											CLASS_HIGH_MAGE)) ? 2 : 4)));
						break;
					case 5:
...
void print_spells(byte * spells, int num, int y, int x, int realm)
{
	...
	for (i = 0; i < num; i++)
	{
		...
		if ((realm + 1 ==
				p_ptr->
				realm1) ? ((spell_forgotten1 & (1L << spell)))
			: ((spell_forgotten2 & (1L << spell))))
		{
			...
			line_attr = TERM_YELLOW;
					...
					p_ptr->
					realm1) ? (spell_learned1 & (1L << spell))
		...
		/* Dump the spell --(-- */
		sprintf(out_val, "  %c) %-30s%2d %4d %3d%%%s", I2A(i),
			spell_names[realm][spell], /* realm, spell */
			s_ptr->slevel, s_ptr->smana, spell_chance(spell, realm),
			comment);
...
bool polymorph_monster(int y, int x)
{
	...
	if (new_r_idx != old_r_idx)
	{
		...
		if (place_monster_aux(y, x, new_r_idx, FALSE, FALSE, friendly,
				pet))
		{
			/* Success */
			polymorphed = TRUE;
		...
		else
		{
			...
			place_monster_aux(y, x, old_r_idx, FALSE, FALSE, friendly,
				pet);
