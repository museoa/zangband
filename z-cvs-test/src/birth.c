...
static hist_type bg[] = {
	...
	{"Your mother was an Ogre, but it is unacknowledged.  ", 25, 74, 20,
			25},
	{"Your father was an Ogre, but it is unacknowledged.  ", 100, 74, 20,
			25},
	...
	{"You are one of several children of a Klackon hive queen.  ", 100, 84,
			85, 50},
	...
	{"You have light blue wings attached to your back, ", 100, 126, 127,
			50},
...
static byte choose_realm(byte choices)
{
	...
	{
		sprintf(buf,
			"Choose a realm (%c-%c), * for random, or = for options: ",
			I2A(0), I2A(n - 1));
		put_str(buf, 20, 2);
		c = inkey();
		if (c == 'Q')
...
static void get_extra(void)
{
	...
	while (TRUE)
	{
		...
		if (player_hp[PY_MAX_LEVEL - 1] < min_value)
			continue;
		if (player_hp[PY_MAX_LEVEL - 1] > max_value)
			continue;
...
static bool get_player_race(void)
{
	...
	{
		sprintf(buf,
			"Choose a race (%c-4), * for random, or = for options: ",
			I2A(0));
		put_str(buf, 17, 2);
		c = inkey();
		if (c == 'Q')
...
static bool get_player_class(void)
{
	...
	{
		sprintf(buf,
			"Choose a class (%c-%c), * for random, or = for options: ",
			I2A(0), I2A(n - 1));
		put_str(buf, 18, 2);
		c = inkey();
		if (c == 'Q')
...
static bool player_birth_aux_1(void)
{
	...
	{
		sprintf(buf,
			"Choose a sex (%c-%c), * for random, or = for options: ",
			I2A(0), I2A(n - 1));
		put_str(buf, 20, 2);
		ch = inkey();
		if (ch == 'Q')
	...
	while (TRUE)
	{
		...
		put_str(format("Number of additional quests? (<%u) ",
		...
		while (TRUE)
		{
			...
			if ((v <= MAX_RANDOM_QUEST - MIN_RANDOM_QUEST + 1) && (v >= 0))
				break;
		}
		break;
	}
	...
	for (i = MIN_RANDOM_QUEST + v - 1; i >= MIN_RANDOM_QUEST; i--)
	{
		...
		for (j = 0; j < MAX_TRIES; j++)
		{
			 ...
			 */
			r_idx =
				get_mon_num(q_ptr->level + 4 + randint(q_ptr->level / 10));
			r_ptr = &r_info[r_idx];

			/* Save the index if the monster is deeper than current monster */
			if (!q_ptr->r_idx ||
				(r_info[r_idx].level > r_info[q_ptr->r_idx].level))
			{
				q_ptr->r_idx = r_idx;
			}
 ...
 */
static int birth_stat_costs[(18 - 10) + 1] =
...
static bool player_birth_aux_2(void)
{
	...
	while (1)
	{
		...
		/* Prompt XXX XXX XXX */
		sprintf(buf,
			"Total Cost %2d/48.  Use 2/8 to move, 4/6 to modify, ESC to accept.",
			cost);
		prt(buf, 0, 0);
				...
				if (flag || (auto_round < last_round + 100))
				{
					...
					for (i = 0; i < A_MAX; i++)
					{
						...
						if (stat_match[i])
						{
							...
							byte attr =
								(p < 100) ? TERM_YELLOW : TERM_L_GREEN;
							sprintf(buf, "%3d.%d%%", p / 10, p % 10);
							c_put_str(attr, buf, 3 + i, col + 13);
						}
