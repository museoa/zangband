...
static bool alloc_stairs(int feat, int num, int walls)
{
	...
	if (feat == FEAT_LESS)
	{
		...
		if (ironman_downward || !dun_level)
			return TRUE;
	}
	else if (feat == FEAT_MORE)
	{
		/* No downstairs on quest levels */
		if ((dun_level > 1) && quest_number(dun_level))
			return TRUE;
...
static void alloc_object(int set, int typ, int num)
{
	...
	for (k = 0; k < num; k++)
	{
		...
		while (dummy < SAFE_MAX_ATTEMPTS)
		{
			...
			/* Require "naked" floor grid */
			if (!cave_naked_bold(y, x) && !((y == py) && (x == px)))
				continue;
...
static bool cave_gen(void)
{
	...
	/* Empty arena levels */
	if (ironman_empty_levels || (empty_levels &&
			(randint(EMPTY_LEVEL) == 1)))
	{
		empty_level = TRUE;
	...
	for (i = 0; i < dun_rooms; i++)
	{
		...
		if (ironman_rooms || (rand_int(DUN_UNUSUAL) < dun_level))
		{
				...
				|| (rand_int(DUN_UNUSUAL) < dun_level))
			{
				...
				if ((k < 18) && room_build(y, x, 5))
					continue;

				/* Type 6 -- Monster pit (10%) */
				if ((k < 28) && room_build(y, x, 6))
					continue;

				/* Type 10 -- Random vault (8%) */
				if ((k < 32) && room_build(y, x, 10))
					continue;
#endif
			...
			if ((k < 75) && room_build(y, x, 2))
				continue;

			/* Type 11 -- Circular (10%) */
			if ((k < 85) && room_build(y, x, 11))
				continue;

			/* Type 12 -- Crypt (15%) */
			if ((k < 100) && room_build(y, x, 12))
				continue;
		}
		...
		if ((k < dun_level) && (!cavern) && (!empty_level) &&
			(laketype == 0))
		{
			/* Type 9 -- Fractal cave */
			if (room_build(y, x, 9))
	...
	for (i = 0; i < dun->cent_n; i++)
	{
		...
		if (pillar_tunnels && (randint(20) > dun_level) &&
			(randint(100) < 25))
		{
	...
	if (cur_hgt < MAX_HGT || cur_wid < MAX_WID)
	{
		...
		{
			msg_format("Reduced monsters base from %d to %d", small_tester,
				i);
		}
	}
	...
	if (ironman_nightmare)
	{
		alloc_object(ALLOC_SET_BOTH, ALLOC_TYP_INVIS,
			randnor(DUN_AMT_INVIS, 3));
	}

	if (empty_level && ((randint(DARK_EMPTY) != 1) ||
			(randint(100) > dun_level)))
	{
		/* Lite the cave */
		for (y = 0; y < cur_hgt; y++)
		{
...
void generate_cave(void)
{
	...
	for (num = 0; TRUE; num++)
	{
		...
		{
			/* Require "goodness" */
			if ((feeling > 9) || ((dun_level >= 5) && (feeling > 8)) ||
				((dun_level >= 10) && (feeling > 7)) || ((dun_level >= 20)
					&& (feeling > 6)) || ((dun_level >= 40) &&
					(feeling > 5)))
			{
		...
		if (okay)
		{
			int i = angtk_generate();
			if (i != -1)
			{
				okay = i;
				if (!okay)
					why = "angband_generate";
			}
		}
#endif /* TNB */
