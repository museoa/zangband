...
static void prt_stat(int stat)
{
	...
	Bind_Generic(EVENT_STAT, stat + 1);
...
static void prt_title(void)
{
	...
	Bind_Generic(EVENT_PY, KEYWORD_PY_TITLE + 1); /* TNB */
	Bind_Status(KEYWORD_STATUS_WINNER + 1);
...
static void prt_level(void)
{
	...
	Bind_Generic(EVENT_PY, KEYWORD_PY_LEVEL + 1);
...
static void prt_exp(void)
{
	...
	Bind_Generic(EVENT_PY, KEYWORD_PY_EXP + 1);
...
static void prt_ac(void)
{
	...
	Bind_Generic(EVENT_PY, KEYWORD_PY_AC + 1);
...
static void prt_hp(void)
{
	...
	Bind_Generic(EVENT_PY, KEYWORD_PY_HP + 1);
...
static void prt_sp(void)
{
	...
	Bind_Generic(EVENT_PY, KEYWORD_PY_SP + 1);
...
static void prt_depth(void)
{
	...
	Bind_Generic(EVENT_PY, KEYWORD_PY_DEPTH + 1);
...
static void prt_hunger(void)
{
	...
	Bind_Status(KEYWORD_STATUS_HUNGER + 1);
...
static void prt_blind(void)
{
	...
	Bind_Status(KEYWORD_STATUS_BLIND + 1);
...
static void prt_confused(void)
{
	...
	Bind_Status(KEYWORD_STATUS_CONFUSED + 1);
...
static void prt_afraid(void)
{
	...
	Bind_Status(KEYWORD_STATUS_AFRAID + 1);
...
static void prt_poisoned(void)
{
	...
	Bind_Status(KEYWORD_STATUS_POISONED + 1);
...
static void prt_state(void)
{
	...
	Bind_Status(KEYWORD_STATUS_STATE + 1);
...
static void prt_speed(void)
{
	...
	Bind_Status(KEYWORD_STATUS_SPEED + 1);
...
static void prt_study(void)
{
	...
	Bind_Status(KEYWORD_STATUS_STUDY + 1);
...
static void prt_cut(void)
{
	...
	Bind_Status(KEYWORD_STATUS_CUT + 1);
...
static void prt_stun(void)
{
	...
	Bind_Status(KEYWORD_STATUS_STUN + 1);
...
static void health_redraw(void)
{
	...
	Bind_Track(KEYWORD_TRACK_HEALTH + 1, p_ptr->health_who, 0, 0);
...
static void prt_frame_basic(void)
{
	...
	Bind_Generic(EVENT_PY, KEYWORD_PY_NAME + 1); /* TNB */
	Bind_Generic(EVENT_PY, KEYWORD_PY_TITLE + 1); /* TNB */
	Bind_Generic(EVENT_PY, KEYWORD_PY_RACE + 1); /* TNB */
}
...
static void calc_spells(void)
{
	...
	for (j = 0; j < 64; j++)
	{
		/* Count known spells */
		if ((j <
				32) ? (spell_learned1 & (1L << j)) : (spell_learned2 & (1L
				   << (j - 32))))
		{
			num_known++;
		}
	...
	for (i = 0; i < 64; i++)
	{
		...
		/* Efficiency -- all done */
		if (!spell_forgotten1 && !spell_forgotten2)
			break;
		...
		/* First set of spells */
		if ((j <
				32) ? (spell_forgotten1 & (1L << j)) : (spell_forgotten2 &
				(1L << (j - 32))))
		{
...
static void calc_mana(void)
{
	...
	if (mp_ptr->spell_book == TV_SORCERY_BOOK)
	{
		...
		if (o_ptr->k_idx && !(f2 & (TR2_FREE_ACT)) && !((f1 & (TR1_DEX)) &&
				(o_ptr->pval > 0)))
		{
			/* Encumbered */
...
static void calc_torch(void)
{
	...
	if (p_ptr->cur_lite == 0 && p_ptr->lite)
		p_ptr->cur_lite = 1;
 ...
 */
static sint add_special_melee_skill(byte pclass, s16b weight,
	object_type * o_ptr)
{
	int add_skill = 0;
...
void redraw_stuff(void)
{
	...
	if (p_ptr->redraw & (PR_MAP))
	{
		p_ptr->redraw &= ~(PR_MAP);
		prt_map();
		p_ptr->redraw |= PR_POSITION; /* Magic Map, etc TNB */
	}
	...
	if (p_ptr->redraw & (PR_TARGET))
	{
		bool change = (target_state & TARGET_STATE_CHANGE) != 0;

		p_ptr->redraw &= ~(PR_TARGET);

		/* Target set */
		if (change || (((target_state & TARGET_STATE_SET) != 0) !=
			(target_who != 0)))
		{
			Bind_Generic(EVENT_TARGET, (target_who ?
				KEYWORD_TARGET_SET : KEYWORD_TARGET_UNSET) + 1);
		}

		/* Target visibility */
		if (change || (((target_state & TARGET_STATE_VIS) != 0) !=
			(target_vis != 0)))
		{
			Bind_Generic(EVENT_TARGET, KEYWORD_TARGET_VISIBILITY + 1);
		}
		...
		if (target_who != 0) target_state |= TARGET_STATE_SET;
		if (target_vis != 0) target_state |= TARGET_STATE_VIS;
	}

	if (p_ptr->redraw & (PR_POSITION))
	{
		p_ptr->redraw &= ~(PR_POSITION);
		Bind_Position(0, 0, 0, py, px);
	}
...
void window_stuff(void)
{
	...
	{
		Bind_Generic(EVENT_TRACK, KEYWORD_TRACK_EQUIPMENT + 1);
	}

	if (p_ptr->window & (PW_INVEN))
	{
		Bind_Generic(EVENT_TRACK, KEYWORD_TRACK_INVENTORY + 1);
	}

	if (p_ptr->window & (PW_MONSTER))
	{
		Bind_Track(KEYWORD_TRACK_RACE + 1, p_ptr->monster_race_idx, 0, 0);
	}

	if (p_ptr->window & (PW_OBJECT))
	{
		Bind_Track(KEYWORD_TRACK_OBJECT + 1, p_ptr->object_kind_idx, 0, 0);
	}

	if (p_ptr->window & (PW_MESSAGE))
	{
		Bind_Generic(EVENT_TRACK, KEYWORD_TRACK_MESSAGE + 1);
	}
