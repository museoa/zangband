...
proc NSAutobar::hook_item {oop message args} {
	...
	Info $oop where $where
		...
		invoke {
			...
			if {[string equal [Info $oop where] floor]} {
...
proc NSAutobar::hook_spell {oop message args} {
	...
	switch -- $message {
		...
		set_list {
			...
			set spellList [angband spell find $bookNum]

			# Keep a list of spell indexes
			set match {}

			# Remember spell status
			set statusList {}
			...
			# Remember spell status
			Info $oop hook,status $statusList

			Info $oop maxWidth $maxWidth
			set fontHeight [font metrics $font -linespace]
			Info $oop maxHeight [expr {[llength $match] * $fontHeight}]
		}
		...
		invoke {
			...
			# Get the spell status
			set status [lindex [Info $oop hook,status] $row]
			switch -- $status {
				study {
					...
					if {[string equal $spellType prayer]} {
						set doSpellChar 0
					}
				}
				cast -
