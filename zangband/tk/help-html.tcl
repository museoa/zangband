...
proc NSHelp::DisplayCmd {oop message first {url ""} {interactive 1}} {
	...
	switch -- $message {
		...
		postDisplay {
			eval $win.frameHtml.xscroll set [$whtml xview]
			eval $win.frameHtml.yscroll set [$whtml yview]
		}
	}
...
proc NSHelp::GeometryCmd {oop} {
	...
	return [GetDefaultGeometry $win main2 main]
}
...
if 1 {
		...
		if {[string equal [file extension $filePath] .html]} {
		...
		if {[string compare [file extension $filePath] .html]} {
			$whtml parse <pre>
		}
...
proc NSHelp::SynchButtons {oop} {
	...
	if {$index > 0} {
		if {[string equal [$back info state] disabled]} {
			$back configure -state normal
		}
	} else {
		if {[string compare [$back info state] disabled]} {
			$back configure -state disabled
		}
	}
	if {$index < $count - 1} {
		if {[string equal [$next info state] disabled]} {
			$next configure -state normal
		}
	} else {
		if {[string compare [$next info state] disabled]} {
...
proc NSHelp::SaveState {oop} {
	...
	set Priv(history,yview) [lreplace $Priv(history,yview) \
		$Priv(history,index) $Priv(history,index) $yview]
...
proc NSHelp::HyperlinkCmd {oop click url} {
	...
	# Mouse entered/left a hyperlink
		...
		if {[string compare [$whtml cget -cursor] $cursor]} {
...
#		set title ""
	...
	if {![string length $title]} {
		if {[string equal $filePath $oldFile]} {
		...
		switch -- $type {
			...
			include {
				if {[scan $string {.%s "%[^"]"} type path] == 2} {
					...
					ReadContents $oop [eval file join $list]
				}
			}
			page {
...
proc NSHelp::SelectPageCmd {oop url data} {
	...
	if {[string compare $fileName1 $fileName]} {
		return 1
	}
...
proc NSHelp::Token_META {oop tokenNumber tagName argList} {
	...
	if {[lsearch -exact [encoding names] $encoding] != -1} {
		...
		if {[string compare [encoding system] $encoding]} {
