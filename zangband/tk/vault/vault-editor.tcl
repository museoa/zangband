...
#	wm transient $win $Windows(main,win)
	...
	set mapId [NSObject::New NSMap $win 32 32 [icon size] [icon size]]
	...
	frame $win.frameRight \
		-borderwidth 0

	frame $win.frameRight.divider1 \
	...
	set map2Id [NSObject::New NSMap $win.frameRight 20 20 4 4]
	...
	set browserId [NSObject::New NSIconBrowser $win.frameRight]
	set canvistId [NSIconBrowser::Info $browserId member,canvistId]
	NSCanvist2::Info $canvistId selectionCmd \
		"NSVaultEditor::SelectionChanged_Icon $oop"
	NSCanvist2::Info $canvistId clickCmd \
		"NSVaultEditor::Click_Icon $oop"
	...
	[NSIconBrowser::Info $browserId groupCanvas] configure \
		-height [expr {($iconH + 8) * $rows}]
	[NSIconBrowser::Info $browserId memberCanvas] configure \
	...
	NSStatusText::StatusText [NSIconBrowser::Info $browserId groupCanvas] \
		$win.statusBar.label0 \
		"List of icon types."
	NSStatusText::StatusText [NSIconBrowser::Info $browserId memberCanvas] \
		$win.statusBar.label0 \
	...
	set frame $win.frameRight.frameFeature
	frame $frame \
		-borderwidth 1 -relief sunken
	set canvistId [NSObject::New NSCanvist $frame $rowHgt $width 100 \
		"NSVaultEditor::NewItemCmd $oop" "NSVaultEditor::HighlightItemCmd $oop"]
	...
	bind $frame.yscroll <Map> "eval %W set \[$canvas yview]"

	# Do something when a row is selected
	NSCanvist::Info $canvistId selectionCmd \
		"NSVaultEditor::SelectionChanged_Feature $oop"
	...
	Info $oop feature,canvistId $canvistId
	...
	grid rowconfigure $win.frameRight 0 -weight 1
	grid rowconfigure $win.frameRight 1 -weight 0
	grid columnconfigure $win.frameRight 0 -weight 0
	grid columnconfigure $win.frameRight 0 -weight 1

	grid $win.frameRight.divider1 \
		-row 0 -column 0 -rowspan 3 -columnspan 1 -sticky ns -padx 4 -pady 2
	grid [NSMap::Info $map2Id frame] \
		-row 0 -column 1 -rowspan 1 -columnspan 1 -sticky news
	grid [NSIconBrowser::Info $browserId frame] \
		-row 1 -column 1 -rowspan 1 -columnspan 1 -sticky news

	# Swap feature list when edit mode changes
	...
	grid $win.frameTool \
		-row 1 -column 0 -sticky n -padx 3 -pady 4 -sticky ns
...
if {$Plugin($ident,file) != ""} {
	...
	if {$mtime > $Plugin($ident,mtime)} {
		...
		if {($Plugin($ident,mode) == "both") || ($Plugin($ident,mode) == [Info $oop editMode])} {
		...
		postDisplay {
			if {$first} {
				...
				if {0 && [angband player depth]} {
					...
					append message "editing a vault to insure that the proper "
...
#	scan [grid bbox $win.frameRight 1 1] "%d %d %d %d" x y width height
	...
	grid columnconfigure $win.frameRight 1 \
...
proc NSVaultEditor::SetupFileMenu {oop} {
	...
	set menu [NSMenu::Info $menuId menu]
	$menu delete 0 end
	set glob [glob -nocomplain [PathTk config *.vlt]]
	set glob [lsort -dictionary $glob]
	set count 0
	foreach path $glob {
		set name [file tail $path]
		$menu add command -label $name -columnbreak [expr {$count % 15 == 0}] \
			-command "NSVaultEditor::MenuInvoke $oop $menuId E_OPEN $path"
		incr count
	}
...
if 0 {
	...
	set menu [NSMenu::Info $menuId menu]
	$menu delete 0 end
	set glob [glob -nocomplain [Path lib edit q*.txt]]
	set glob [lsort -dictionary $glob]
	set count 0
	foreach path $glob {
		set name [file tail $path]
		...
		$menu add command -label $name -columnbreak [expr {$count % 15 == 0}] \
			-command "NSVaultEditor::MenuInvoke $oop $menuId E_QUEST $path"
		incr count
	}
	if {[$menu index end] != "none"} {
		$menu add separator
	...
	$menu delete 0 end
	set pattern t*.txt
	set glob [glob -nocomplain [Path lib edit $pattern]]
	set glob [lsort -dictionary $glob]
	set count 0
	foreach path $glob {
		set name [file tail $path]
		...
		$menu add command -label $name -columnbreak [expr {$count % 15 == 0}] \
			-command "NSVaultEditor::MenuInvoke $oop $menuId E_TOWN $path"
		incr count
	}
	if {[$menu index end] != "none"} {
		$menu add separator
...
proc NSVaultEditor::SelectionChanged_Icon {oop canvistId select deselect} {
...
proc NSVaultEditor::Click_Icon {oop canvistId index} {
...
proc NSVaultEditor::ExamineCmd {oop widgetId y x} {
	...
	if {[NSWidget::Info [Info $oop map,widgetId] scale] == [icon size]} {
...
proc NSVaultEditor::ExamineCmd_Map {oop widgetId y x} {

	set mapId [Info $oop mapId]

	if {[NSWidget::Info $widgetId scale] != [icon size]} {
	...
	ExamineCmd $oop [Info $oop widgetId] $y $x
	Info $oop ignoreExamine 0
...
proc NSVaultEditor::SetLetters {oop theList} {
	...
	foreach {char feat F aschar} $theList {
		...
		if {!$aschar && (![llength $F] || ([llength $F] == 3))} {
			...
			# Foreground
			set assign [assign set feature $feat]
...
#					set icon [lindex [alternate get $iconType] 0]
			...
			# Background
			set featBG [feature configure $feat -background]
...
if 0 {
			...
			NSCanvist::DeleteAll [Info $oop feature,canvistId]
		}
		MODE_VAULT -
		MODE_TOWN {
			Info $oop mode,default 1
...
#	ToUndo $oop
	...
	switch -- $mode {
		...
		feature {
			...
			# first time
			if {![NSCanvist::Info [Info $oop feature,canvistId] count]} {
				SetFeatureList $oop
			}

			Info $oop save,icon [Info $oop tool,icon]

			set selection [NSCanvist::Selection [Info $oop feature,canvistId]]
			if {[llength $selection]} {
				set row [lindex $selection 0]
				NSCanvist::RemoveSelection [Info $oop feature,canvistId]
			} else {
				set row 0
			}

			# Select row so tool icon set properly
			NSCanvist::UpdateSelection [Info $oop feature,canvistId] $row all
		}
	}
...
proc NSVaultEditor::DisplayIcon {oop which} {
	...
	switch -- [Info $oop editMode] {
		...
		feature {
			set canvistId [Info $oop feature,canvistId]
			set letter [Info $oop tool,letter]
...
proc NSVaultEditor::SetFeatureList {oop} {
	...
	foreach f_idx $row2feat ass $row2ass assbg $row2assbg name $row2name {
		...
		lappend itemList [list $ass $assbg $name]
	}
...
proc NSVaultEditor::NewItemCmd {oop canvistId y assign assignbg text} {
	...
	lappend itemIdList [$c create widget [expr {4 + $iw / 2}] \
		[expr {$y + 4 + $ih / 2}] -anchor center \
		-assign $assign -assignbg $assignbg -tags icon]
...
# NSVaultEditor::HighlightItemCmd --
#
...
proc NSVaultEditor::HighlightItemCmd {oop canvistId state args} {
...
proc NSVaultEditor::SelectionChanged_Feature {oop canvistId select deselect} {
...
proc NSVaultEditor::VaultToText {oop} {
	...
	if {![winfo exists $win]} {
		...
		text $frame.text \
			...
			-width 82 -height 24 -font [Global font,fixed,small] -borderwidth 0 \
			-highlightthickness 0
...
proc NSVaultEditor::AskForSize {oop} {
	...
	if {![winfo exists $w]} {
		...
		button $f.btn2 -text Cancel -command "set NSVaultEditor::Result 0" \
			-width 9
		pack $f.btn1 -side left
		pack $f.btn2 -side left -padx 10 -pady 10
...
proc NSVaultEditor::ReadPluginIndex {} {
...
proc NSVaultEditor::Plugin {ident file label opt} {
	...
	if {[string length $file]} {
		source [CPathTk vault plugins $file]
...
proc NSVaultEditor::OpenQuest {oop} {
	...
	set glob [glob -nocomplain [Path lib edit {q[0-9]*.txt}]]
	set glob [lsort -dictionary $glob]
