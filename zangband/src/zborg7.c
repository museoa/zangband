...
bool borg_test_stuff(bool star_id)
{
    ...
    for (i = 0; i < INVEN_PACK; i++)
    {
        ...
        /* weak pseudo id */
        if (strstr(item->desc, "{good") && (borg_class == CLASS_MAGE ||
            borg_class == CLASS_PRIEST || borg_class == CLASS_RANGER)) v = item->value + 10000L;
        /* heavy pseudo id */
        else if (strstr(item->desc, "{good") && borg_gold < 10000) v = item->value +1000L;
        else if (strstr(item->desc, "{excellent")) v = item->value + 20000L;
        else if (strstr(item->desc, "{special")) v = item->value + 50000L;
        else if (strstr(item->desc, "{terrible")) v = item->value + 50000L;
        else if (strstr(item->desc, "{indestructible")) v = item->value + 50000L;
        /* Nothing */
        if (!v) continue;
    ...
    for (i = 0; i < INVEN_PACK; i++)
    {
        ...
        if (strstr(item->desc, "{average")) continue;
...
bool borg_swap_rings(void)
{
    ...
    if (borg_items[INVEN_LEFT].cursed ||
        borg_items[INVEN_RIGHT].cursed) return (FALSE);
...
static void borg_best_stuff_aux(int n, byte *test, byte *best, s32b *vp)
{
    ...
    for (i = 0; i < ((shop_num == BORG_HOME) ? (INVEN_PACK + STORE_INVEN_MAX) : INVEN_PACK); i++)
    {
        borg_item *item;
        if (i < INVEN_PACK)
            item = &borg_items[i];
