...
{
	static char *cmdOptions[] = {"acquirement", "alloc_artifact", "auto_lite",
...
{
	static char *cmdOptions[] = {"command", "create_item", "summon_specific",
	...
	switch (index)
	{
		...
		case 2: /* summon_specific */
		{
			...
			for (i = 3; i < objc; )
			{
				static char *cmdOptions[] = {"-friend", "-group", "-pet",
					"-sleep", NULL};
			...
			if (i == 20)
			{
				Tcl_SetStringObj(resultPtr, "couldn't pick location to alloc", -1);
				return TCL_ERROR;
			}
...
{
	char *cmdOptions[] = {"bindings", "commands", "widgets", "vault", NULL};
	...
	switch (option)
	{
		...
		{
			static char *vaultOption[] = {"max", "info", NULL};
			int index, v_idx;
			vault_type *v_ptr;
			char *arrayName;
			Tcl_Obj *namePtr;
