 ...
 */
#define PORTVERSION	"1.29-dev (2003-07-27)"
 ...
 */
#define RISCOS_VARIANT	"Zangband"

/*
 * AUTHORS
 *  For the info box. [eg. "Ben Harrison"]
 */
#define AUTHORS		"Zangband dev team"
...
#define PDEADCHK	(p_ptr->state.is_dead)
...
{
	/* Really no need to check the alarm more than twice per second. */
	if (alarm_type && Time_Monotonic() > alarm_lastcheck + 200)
	{
		check_alarm();
	}
...
static void enter_fullscreen_mode(void)
{
	...
	old_mouse_posn[0] = 4;
	SWI(2, 0, SWI_OS_Word, 21, &old_mouse_posn);
...
static void do_alarm_message_input(int y)
{
	...
	do
	{
	    put_fstr(26, y, CLR_YELLOW "%-51s", alarm_message);
	...
	put_fstr(26, y, CLR_WHITE "%-51s", alarm_message);
}


#define tum_col(X)  ((X) ? CLR_L_BLUE : CLR_WHITE )
...
static errr Term_user_acn(int n)
{
	...
	do
	{
		redraw_mung = 0;
		Term_clear();
		put_fstr(2, 1, CLR_YELLOW "%s %s", VARIANT, VERSION);
		put_fstr(2, 2, CLR_SLATE "Front-end %s", PORTVERSION);
		put_fstr(2, 4, CLR_WHITE
					 "Use cursor up/down to select an option then cursor left/right to alter it.");
		put_fstr(2, 5, CLR_WHITE
					 "Hit 'S' to save these settings (alarm settings are saved automatically).");
		put_fstr(2, 6, CLR_WHITE "Hit ESC to return to the game.");

		for (k = 0; k < 32; k++)
		{
			Term_putch(31 + k + (k / 2), 8, k / 2, '#');
		}

		do
		{
			/* Gamma value never goes about 9.00, 5 char array should be fine */
			char gammas[5];
			sprintf(gammas, "%.2lf", gamma);

			put_fstr(2, 8,
						 "%s     Gamma correction : %s", tum_col(optn == 0), gammas);
			put_fstr(2, 9, "%s     Force monochrome : %s",
						 tum_col(optn == 1), tum_onoff(force_mono));
			put_fstr(2, 10, "%s        Sound effects : %s",
						 tum_col(optn == 2), tum_onoff(enable_sound));
			put_fstr(2, 11, "%s  Sound effect volume : ", tum_col(optn == 3));
			put_fstr(26, 11, "%s%-3d",
						 sound_volume > 127 ? CLR_RED : tum_col(optn == 3),
						 sound_volume);
			put_fstr(30, 11, "%s(127 = full volume)", tum_col(optn == 3));
			put_fstr(2, 12, "%s     Start fullscreen : %s",
						 tum_col(optn == 4), tum_onoff(start_fullscreen));
			put_fstr(30, 12, 
						 "%s(also selects fullscreen/desktop now)", tum_col(optn == 4));
			put_fstr(2, 13, "%s        Use hourglass : %s",
						 tum_col(optn == 5), tum_onoff(use_glass));
			put_fstr(2, 14,
						 "%s'Hard' input flushing : %s", tum_col(optn == 6), tum_onoff(hack_flush));

			put_fstr(7, 16, "%s      Alarm type : %-20s",
						 tum_col(optn == 7), alarm_types[alarm_type]);
			put_fstr(7, 17, CLR_WHITE "            Time : ");
			put_fstr(26, 17, "%s%02d", tum_col(optn == 8), alarm_h);
			put_fstr(28, 17, CLR_WHITE, ":");
			put_fstr(29, 17, "%s%02d", tum_col(optn == 9), alarm_m);
			put_fstr(7, 18, "%s         Message : %-51s",
						 tum_col(optn == 10), alarm_message);
			put_fstr(7, 19, "%s            Beep : %s",
						 tum_col(optn == 11), tum_onoff(alarm_beep));

#ifdef FE_DEBUG_INFO
			put_fstr(2, 23, "%sShow debug info", tum_col(optn == 23));
			max_opt = 12;
#endif
			...
			switch (k)
			{
				...
				case 's':  case 'S':
					save_choices();
					put_fstr(2, 23, CLR_YELLOW "Options saved.       ");
					Term_fresh();
...
static void debug_print_line(char *l)
{
	...
	if (debug_cy > 22)
	{
		...
		{
			int k;
			put_fstr(0, 23, CLR_YELLOW "[RET one line, SPC one page]");
			do
			{
				k = inkey();
			}
	...
	/* display text */
	put_fstr(debug_cx, debug_cy, "$%c%.*s", 'A' + debug_cl, l);
