...
static int Struct_FindFieldByName(Tcl_Interp *interp, char *fieldName, Tcl_HashTable *tablePtr, int *fieldIndex)
{
	...
	if ((hPtr = Tcl_FindHashEntry(tablePtr, fieldName)) == NULL)
	{
		...
		Tcl_AppendStringsToObj(resultPtr, "unknown field \"", fieldName,
			"\"", NULL);
...
static int Struct_FindTypeByName(Tcl_Interp *interp, char *typeName, Tcl_HashTable *tablePtr, int *typeIndex)
{
	...
	if ((hPtr = Tcl_FindHashEntry(tablePtr, typeName)) == NULL)
	{
		...
		/* Append an error string */
		Tcl_AppendStringsToObj(resultPtr, "unknown type \"", typeName,
			"\"", NULL);
	...
	int *elemIndex, Tcl_Obj *objPtr)
{
	...
	if (((*elemIndex) < 0) || ((*elemIndex) >= typePtr->max))
	{
		...
		(void) sprintf(errorMsg, 
			"bad array index \"%d\": must be from 0 to %d",
	...
	Tcl_Obj *CONST objv[], int objOffset)
{
	...
	char *t, **field_names, errorMsg[128];
	int i, j, index = 0;
		...
		for (i = 0; i < request_field; i++)
		{
			...
			switch (fieldPtr->type)
			{
				...
				default:
					...
					{
						Tcl_AppendStringsToObj(Tcl_GetObjResult(interp),
							"illegal op \"", fieldOp[field_op[i]],
							"\" on field \"", fieldPtr->text, "\"",
							NULL);
						return TCL_ERROR;
					}
...
bad_index:
	(void) sprintf(errorMsg, "bad index \"%d\": must be between 0 and %d",
		index, typePtr->max - 1);
...
/*	int objC = objc - objOffset; */
	...
	switch (fieldPtr->type)
	{
		...
		default:
			...
			/* Set the error */
			(void) sprintf(errorMsg, "field \"%s\" not flags", t);
			Tcl_SetResult(interp, errorMsg, TCL_VOLATILE);
	...
	{
		/* Set the error */
		(void) sprintf(errorMsg, "field \"%s\" has no flag names", t);
		Tcl_SetResult(interp, errorMsg, TCL_VOLATILE);
	...
	int objc, Tcl_Obj *CONST objv[], int objOffset)
{
	...
	if (objC > 0)
	{
		...
		else if ((fieldIndex < 0) || (fieldIndex >= typePtr->count))
		{
			/* Set the error */
			(void) sprintf(errorMsg, 
				"bad field index \"%d\": must be from 0 to %d",
...
int Struct_Init(Tcl_Interp *interp)
{
	...
	g_struct = Array_New(1, sizeof(StructType));
	g_struct_count = 0;
