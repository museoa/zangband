...
bool monst_spell_monst(int m_idx)
{
	...
	for (i = 1; i < m_max; i++)
	{
		...
		if ((RF_FLAG(r_ptr->flags, 1, SMART)) &&
		...
		switch (thrown_spell)
		{
			...
			case 128 + 10:
			{
				...
				if ((RF_FLAG(tr_ptr->flags, 0, UNIQUE)) ||
					(RF_FLAG(tr_ptr->flags, 2, NO_CONF)) ||
					(tr_ptr->level > randint1(rlev * 3) / 2))
				{
					...
					{
						/* Memorize a flag */
						if (TEST_FLAG(tr_ptr->flags, 2, RF2_NO_CONF))
						{
			...
			case 128 + 27:
			{
				...
				if (RF_FLAG(tr_ptr->flags, 2, NO_FEAR))
				{
			...
			case 128 + 28:
			{
				...
				if (RF_FLAG(tr_ptr->flags, 2, NO_CONF))
				{
			...
			case 128 + 31:
			{
				...
				if ((RF_FLAG(tr_ptr->flags, 0, UNIQUE)) ||
					(RF_FLAG(tr_ptr->flags, 2, NO_STUN)))
				{
					if (see_t) msgf("%^s is unaffected.", t_name);
				}
				else if (tr_ptr->level > randint1(rlev * 3) / 2)
			...
			case 160 + 9:
			{
				...
				if (RF_FLAG(tr_ptr->flags, 2, RES_TELE))
				{
					if (RF_FLAG(tr_ptr->flags, 0, UNIQUE))
					{
						if (see_t)
