...
static void borg_flow_spread(int depth, bool optimize, bool avoid, bool tunneling)
{
    ...
    while (flow_head != flow_tail)
    {
        ...
        for (i = 0; i < 8; i++)
        {
            ...
            if (mb_ptr->terrain >= FEAT_TRAP_TRAPDOOR && mb_ptr->terrain <= FEAT_TRAP_SLEEP &&
...
static bool borg_surrounded(void)
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        if ((mb_ptr->terrain >= FEAT_SHOP_HEAD) && (mb_ptr->terrain <= FEAT_SHOP_TAIL)) non_safe_grids ++;

        /* Mega-Hack -- skip traps XXX XXX XXX */
        if ((mb_ptr->terrain >= FEAT_TRAP_TRAPDOOR) && (mb_ptr->terrain <= FEAT_TRAP_SLEEP)) non_safe_grids ++;
#endif /* 0 */
    }
...
static void borg_near_monster_type(int dist)
{
    ...
    for (i = 1; i < borg_kills_nxt; i++)
    {
        ...
        if (borg_skill[BI_CLEVEL] <= 40 &&
            (strstr(r_name + r_ptr->name, "Greater hell"))) scaryguy_on_level = TRUE;
...
static bool borg_dim_door(int emergency, int p1)
{
    ...
    borg_note(format("# Dim Door: Safest grid: (%d, %d) with %d Danger",b_y, b_x, b_p));
	dim_door_y = b_y;
	dim_door_x = b_x;
...
static bool borg_escape(int b_q)
{
           ...
           borg_class == CLASS_WARRIOR) ))
    {
        ...
        /* time to flee */
        if (!goal_fleeing && (!borg_fighting_unique || borg_skill[BI_CLEVEL] < 35) &&
        ...
        /* Flee now */
        if (!goal_leaving && (!borg_fighting_unique || borg_skill[BI_CLEVEL] < 35) &&
            !vault_on_level && !borg_quest_level)
        {
            /* Flee! */
...
static bool borg_heal(int danger )
{
   ...
   /*  Hack -- rest until healed */
    if ( (!borg_skill[BI_ISBLIND] && !borg_skill[BI_ISPOISONED] && !borg_skill[BI_ISCUT] && !borg_goi &&
    ...
    if (borg_fighting_unique >= 10)
    {
        if (borg_skill[BI_CURHP] <= 625 && borg_fighting_unique >= 10 &&
            ((borg_skill[BI_CURHP] > 250 && borg_spell_fail(REALM_LIFE,2,6, 14)) ||  /* Holy Word */
            ...
            /* Choose Life over Healing if way down on pts*/
             ...
             borg_quaff_potion(SV_POTION_STAR_HEALING) ||
             borg_quaff_potion(SV_POTION_HEALING) ||
             borg_spell_fail(REALM_LIFE,2,6, 17) ||  /* Holy Word */
...
/* True Vampirism ? */
    ...
    if (borg_skill[BI_CURSP] < (borg_skill[BI_MAXSP] / 10) ||
       ((borg_skill[BI_CURSP] < 70 && borg_skill[BI_MAXSP] > 200) && (borg_goi <= borg_game_ratio*3)) )
    {
    ...
    if (hp_down < 250 &&
        ...
        ( ((!borg_skill[BI_ATELEPORT] ||
           borg_skill[BI_DEV]-borg_items[borg_slot(TV_ROD, SV_ROD_HEALING)].level > 7) &&
    ...
    /* Heal step two (300hp) */
    if (hp_down < 350 &&
        danger/2 < borg_skill[BI_CURHP] +300 &&
        (borg_use_staff_fail(SV_STAFF_HEALING) ||
         (borg_fighting_evil_unique && borg_spell_fail(REALM_LIFE,2,6, allow_fail)) || /* holy word */
/* Vamp Drain ? */
		 borg_use_staff_fail(SV_STAFF_HOLINESS) ||
         borg_spell_fail(REALM_LIFE,1, 6, allow_fail) ||
         ((!borg_skill[BI_ATELEPORT] ||
    ...
    if (hp_down < 650 &&
        ...
        ((borg_fighting_evil_unique && borg_spell_fail(REALM_LIFE,2,6, allow_fail)) || /* holy word */
/* Vamp Drain ? */
         ((!borg_skill[BI_ATELEPORT] || borg_skill[BI_DEV]-borg_items[borg_slot(TV_ROD, SV_ROD_HEALING)].level > 7) &&
           borg_zap_rod(SV_ROD_HEALING)) ||
         borg_spell_fail(REALM_LIFE,1, 6, allow_fail) ||
     ...
     */
    if (hp_down >= 650 && (danger/2 < borg_skill[BI_CURHP] +500)  &&
        ((borg_fighting_evil_unique && borg_spell_fail(REALM_LIFE,2,6, allow_fail)) || /* holy word */
/* Vamp Drain ? */
...
bool borg_caution(void)
{
    ...
    if (borg_goi || (p > avoidance / 10) || (p > borg_fear_region[c_y/11][c_x/11]))
    {
        /* Describe (briefly) the current situation */
        borg_note(format("# Loc:%d,%d Dep:%d Lev:%d HP:%d/%d SP:%d/%d Danger:p=%d",
                         c_y, c_x, borg_skill[BI_CDEPTH], borg_skill[BI_CLEVEL],
                         borg_skill[BI_CURHP], borg_skill[BI_MAXHP], borg_skill[BI_CURSP], borg_skill[BI_MAXSP],
                         p));
        if (borg_goi)
        {
    ...
    if (goal_recalling && goal_recalling <= (borg_game_ratio *2) && !borg_skill[BI_CDEPTH])
    {
        ...
        if ((!borg_speed && borg_spell_fail(REALM_SORCERY,1, 5,15)) ||
            ...
            (my_oppose_fire + my_oppose_cold + my_oppose_elec < 2 &&
             borg_spell_fail(REALM_NATURE,0,6,15)) ||
            (!borg_shield && !borg_goi && borg_spell_fail(REALM_NATURE,2,2,15)) ||
    ...
    else if (p > (borg_skill[BI_CURHP] * 2))
    {
        ...
        if (!goal_fleeing && !borg_fighting_unique && (borg_skill[BI_CLEVEL] < 50) &&
            !vault_on_level && !borg_quest_level && (borg_skill[BI_CDEPTH] < 100 && borg_ready_morgoth == 1))
        {
            /* Note */
          ...
          (borg_surround && p != 0))
   {
        ...
        for (j = 1; j < borg_view_n; j++)
        {
            ...
            while (1)
            {
                ...
                if (r == 1)
                {
                    ...
                    {
                        if (borg_danger(y2, x2, 1, TRUE) >= (b_r  <= 3 ? avoidance * 15/10 : avoidance))
                        break;
                    }
                }
...
#if 0
                /* Skip traps */
                if ((mb_ptr->terrain >= FEAT_TRAP_TRAPDOOR) && (mb_ptr->terrain <= FEAT_TRAP_SLEEP)) break;
#endif /* 0 */
                /* Safe arrival */
                if ((x1 == x2) && (y1 == y2))
                {
        ...
        if (b_r >= 0)
        {
            ...
            borg_note(format("# Retreating to %d,%d (distance %d) via %d,%d (%d > %d)",
                             b_y, b_x, b_r, g_y, g_x, p, borg_danger(g_y, g_x, 2, TRUE)));
        ...
        (borg_surround && p != 0))
    {
        ...
        for (i = 0; i < 8; i++)
        {
...
#if 0
            /* Mega-Hack -- skip stores XXX XXX XXX */
            if ((mb_ptr->terrain >= FEAT_SHOP_HEAD) && (mb_ptr->terrain <= FEAT_SHOP_TAIL)) continue;

            /* Mega-Hack -- skip traps XXX XXX XXX */
            if ((mb_ptr->terrain >= FEAT_TRAP_TRAPDOOR) && (mb_ptr->terrain <= FEAT_TRAP_SLEEP)) continue;
#endif /* 0 */
            ...
            if (b_k == k)
            {
                ...
                if (borg_skill[BI_CLEVEL] <= 3 && (borg_skill[BI_CURHP] < borg_skill[BI_MAXHP] ||
...
static int borg_thrust_damage_one(int i)
{
    ...
    if (borg_class == CLASS_MAGE && borg_skill[BI_MAXCLEVEL] < 40) dam = dam * 6 / 10;
...
bool borg_target_unknown_wall(int y, int x)
{
    ...
    while (1)
    {
        map_block *mb_ptr = map_loc(n_x, n_y);
 ...
 */
static int borg_attack_aux_spell_bolt(int realm, int book, int what, int rad, int dam, int typ)
{
    int b_n;
    int penalty =0;
    ...
    if (borg_skill[BI_CURSP] - as->power < borg_skill[BI_MAXSP] / 2) b_n = b_n - (as->power * 10);

    /* Penalize use of deep reserve mana */
    if (borg_skill[BI_CURSP] - as->power < borg_skill[BI_MAXSP] / 3) b_n = b_n - (as->power * 20);
 ...
 */
static int borg_attack_aux_spell_bolt_reserve(int realm, int book, int what, int rad, int dam, int typ)
{
    int b_n;
    int i;
    ...
    for (i = 1; i < borg_kills_nxt; i++)
    {
        ...
        /* check the location */
        if (borg_temp_x[0] != kill->x ||
            borg_temp_y[0] != kill->y)
            continue;
 ...
 */
static int borg_attack_aux_spell_dispel(int realm, int book, int what, int rad, int dam, int typ)
{
    int b_n;
    int penalty =0;
 ...
 * Take into account the failure rate of spells/objects/etc.  XXX XXX XXX
 */
static int borg_attack_aux_mind_bolt(int spell, int level, int rad, int dam, int typ)
{
    int b_n = -1;
    int penalty = 0;
...
static int borg_attack_aux(int what)
{
    ...
    switch (what)
    {
        ...
        return (borg_attack_aux_spell_dispel(REALM_LIFE,0,4, rad, dam, GF_LITE_WEAK));

        /* Spell -- Holy Orb */
        case BF_LIFE_HOLY_ORB:
        dam = ((3*3) + borg_skill[BI_CLEVEL] + (borg_skill[BI_CLEVEL] / 3)) ;
        rad = 2;
        return (borg_attack_aux_spell_bolt(REALM_LIFE,1,4, rad, dam, GF_HOLY_FIRE));

        /* Spell -- Exorcism */
        case BF_LIFE_EXORCISM:
        dam = borg_skill[BI_CLEVEL] /2;
		rad = MAX_SIGHT + 10;
        return (borg_attack_aux_spell_dispel(REALM_LIFE,2,0, rad, dam, GF_DISP_UNDEAD_DEMON));

        /* Spell -- Disp Undead & Demon */
        case BF_LIFE_DISP_UNDEAD:
        dam = (borg_skill[BI_CLEVEL] * 3) /2;
		rad = MAX_SIGHT + 10;
        return (borg_attack_aux_spell_dispel(REALM_LIFE,2, 2, rad, dam, GF_DISP_UNDEAD_DEMON));

        /* Spell -- Disp Evil */
        case BF_LIFE_DISP_EVIL:
        dam = (borg_skill[BI_CLEVEL] * 4) /2;
        rad = MAX_SIGHT + 10;
        return (borg_attack_aux_spell_dispel(REALM_LIFE,2, 4, rad, dam, GF_DISP_EVIL));
          ...
          */
        {
         ...
         return (borg_attack_aux_spell_dispel(REALM_LIFE,2,6, rad, dam, GF_DISP_EVIL));
        }
        ...
        if (borg_skill[BI_MAXHP] - borg_skill[BI_CURHP] >= 200) dam = (dam * 12) / 10;
        /* if no speedy, add bonus */
        if (!borg_speed) dam = (dam * 13) / 10;
        /* bonus for refreshing the speedy */
        if (borg_speed) dam = (dam * 11) / 10;
        return (borg_attack_aux_spell_dispel(REALM_LIFE,3,6, rad, dam, GF_HOLY_FIRE));
        ...
        dam = (3 + ((borg_skill[BI_CLEVEL] - 1) / 5)) / 2;
        return (borg_attack_aux_spell_bolt_reserve(REALM_ARCANE,0, 0, rad, dam, GF_ELEC));

        /* Spell -- Light Area */
        case BF_ARCANE_LAREA:
        dam = (2 * (borg_skill[BI_CLEVEL])/2)/2;
        rad = borg_skill[BI_CLEVEL] +1 + 10;
        return (borg_attack_aux_spell_dispel(REALM_ARCANE,0, 5, rad, dam, GF_LITE_WEAK));

        /* Spell -- Stone to Mud */
        case BF_ARCANE_STONEMUD:
        dam = 45;
        return (borg_attack_aux_spell_bolt(REALM_ARCANE,2, 4, rad, dam, GF_KILL_WALL));

        /* Spell -- Light Beam */
        case BF_ARCANE_LBEAM:
        dam = 30;
        rad = -1;
        return (borg_attack_aux_spell_bolt(REALM_ARCANE,2, 5, rad, dam, GF_LITE_WEAK));

        /* Spell -- Elem Ball */
        case BF_ARCANE_ELEM_BALL:
        dam = (75 + borg_skill[BI_CLEVEL]);
        return (borg_attack_aux_spell_bolt(REALM_ARCANE,2, 6, rad, dam, GF_ELEMENTS));

        /* Spell -- Light area */
        case BF_SORC_LAREA:
        dam = (2 * (borg_skill[BI_CLEVEL])/2)/2;
        rad = borg_skill[BI_CLEVEL] +1 + 10;
        return (borg_attack_aux_spell_dispel(REALM_SORCERY,0, 3, rad, dam, GF_LITE_WEAK));

        /* Spell -- Confuse Monster */
        case BF_SORC_CONF_MON:
        dam = 10;
        return (borg_attack_aux_spell_bolt(REALM_SORCERY,0, 4, rad, dam, GF_OLD_CONF));

        /* Spell -- Sleep I */
        case BF_SORC_SLEEP_I:
        dam = 10;
        return (borg_attack_aux_spell_bolt(REALM_SORCERY,0, 6, rad, dam, GF_OLD_SLEEP));

        /* Spell -- Slow I */
        case BF_SORC_SLOW_MON:
        dam = 10;
        return (borg_attack_aux_spell_bolt(REALM_SORCERY,1, 2, rad, dam, GF_OLD_SLOW));

        /* Spell -- Sleep III */
        case BF_SORC_SLEEP_III:
        rad = MAX_SIGHT+10;
        dam = 10;
        return (borg_attack_aux_spell_dispel(REALM_SORCERY, 1, 3, rad, dam, GF_OLD_SLEEP));

        /* Spell -- Stasis */
        case BF_SORC_STASIS:
        dam = 30;
        return (borg_attack_aux_spell_bolt(REALM_SORCERY, 3, 0, rad, dam, GF_STASIS));
        ...
        dam = 3 * ((borg_skill[BI_CLEVEL]-1) /5);
        return (borg_attack_aux_spell_bolt_reserve(REALM_TRUMP,0, 1, rad, dam, GF_PSI));
		...
		dam = (borg_skill[BI_CLEVEL] / 2);
        return (borg_attack_aux_spell_dispel(REALM_NATURE,0, 4, rad, dam, GF_LITE_WEAK));

        /* Spell -- Stone to Mud */
        case BF_NATURE_STONEMUD:
        dam = (45);
        return (borg_attack_aux_spell_bolt(REALM_NATURE,1, 0, rad, dam, GF_KILL_WALL));
        ...
        dam = (6 * 8) / 2;
        rad = -1;
        return (borg_attack_aux_spell_bolt(REALM_NATURE,1, 4, rad, dam, GF_LITE_WEAK));

        /* Spell -- Entangle */
        case BF_NATURE_ENTANGLE:
        rad = 10 + MAX_SIGHT;
        dam = (10);
        return (borg_attack_aux_spell_dispel(REALM_NATURE,1, 5, rad, dam, GF_OLD_SLOW));
        ...
        return (borg_attack_aux_spell_bolt(REALM_NATURE, 3, 2, rad, dam, GF_OLD_CONF));
        ...
        rad = 8 + 10;
        if (borg_skill[BI_FEAR_LITE] &&
            borg_skill[BI_CURHP] < 150) dam = 0;
        return (borg_attack_aux_spell_dispel(REALM_NATURE, 3,5, rad, dam, GF_LITE_WEAK));

        /* Natures Wrath */
        case BF_NATURE_NATWRATH:
        dam = ((borg_skill[BI_CLEVEL] + 100));
        rad = (borg_skill[BI_CLEVEL]/12) + 1 + 10;
        return (borg_attack_aux_spell_dispel(REALM_NATURE, 3,7, rad, dam, GF_DISINTEGRATE));

        /* Magic Missile */
        case BF_CHAOS_MMISSILE:
        dam = (3 + ((borg_skill[BI_CLEVEL] - 1) / 5)) * 2;
        return (borg_attack_aux_spell_bolt(REALM_CHAOS,0,0, rad, dam, GF_MISSILE));
        ...
        rad = (borg_skill[BI_CLEVEL] / 10)+1 + 10;
        dam = (borg_skill[BI_CLEVEL] / 2);
        return (borg_attack_aux_spell_dispel(REALM_CHAOS,0,2, rad, dam, GF_LITE_WEAK));
        ...
        return (borg_attack_aux_spell_bolt(REALM_CHAOS,0,2, rad, dam, GF_FIRE));

        /* Fist of Force */
        case BF_CHAOS_FISTFORCE:
        dam = (8 + ((borg_skill[BI_CLEVEL] - 5) / 4)) * 4;
        return (borg_attack_aux_spell_bolt(REALM_CHAOS,0,6, rad, dam, GF_DISINTEGRATE));
        ...
        return (borg_attack_aux_spell_dispel(REALM_CHAOS,1,2, rad, dam, GF_SOUND));
        ...
        return (borg_attack_aux_spell_bolt(REALM_CHAOS,1,4, rad, dam, GF_LITE_WEAK));
        ...
        dam = 10;
        return (borg_attack_aux_spell_bolt(REALM_CHAOS,2,0, rad, dam, GF_OLD_POLY));
        ...
        return (borg_attack_aux_spell_bolt(REALM_CHAOS,2,3, rad, dam, GF_DISINTEGRATE));

        /* Gravity */
        case BF_CHAOS_GRAVITY:
        rad = -1;
        dam = (9 + ((borg_skill[BI_CLEVEL] - 5) / 4)) * 4;
        return (borg_attack_aux_spell_bolt(REALM_CHAOS,3,0, rad, dam, GF_GRAVITY));

        /* Meteor Swarm */
        case BF_CHAOS_METEORSWARM:
        rad = 3;
        dam = (borg_skill[BI_CLEVEL] + 65);
        return (borg_attack_aux_spell_bolt(REALM_CHAOS,3,1, rad, dam, GF_METEOR));
        ...
        rad = 0;
        dam = (3 + ((borg_skill[BI_CLEVEL] - 1) / 5))/2;
        return (borg_attack_aux_spell_bolt(REALM_DEATH,0,1, rad, dam, GF_HELL_FIRE));
        ...
        dam = 10;
        return (borg_attack_aux_spell_bolt(REALM_DEATH,0,4, rad, dam, GF_OLD_SLEEP));

        /* Horrify */
        case BF_DEATH_HORRIFY:
        dam = 10;
        return (borg_attack_aux_spell_bolt(REALM_DEATH,0,6, rad, dam, GF_TURN_ALL));

        /* Entropy */
        case BF_DEATH_ENTROPY:
        rad = 2;
        dam = 10 + borg_skill[BI_CLEVEL] + (borg_skill[BI_CLEVEL] / 2);
        return (borg_attack_aux_spell_bolt(REALM_DEATH,1,0, rad, dam, GF_OLD_DRAIN));

        /* Nether Bolt */
        case BF_DEATH_NETHERBOLT:
        dam = (6 + ((borg_skill[BI_CLEVEL] - 5)) / 4);
        return (borg_attack_aux_spell_bolt(REALM_DEATH,1,1, rad, dam, GF_HELL_FIRE));

        /* Terror */
        case BF_DEATH_TERROR:
        dam = borg_skill[BI_CLEVEL] +30;
        return (borg_attack_aux_spell_bolt(REALM_DEATH,1,2, rad, dam, GF_TURN_ALL));

        /* Vamp Drain */
      	case BF_DEATH_VAMPDRAIN:
      	dam = (borg_skill[BI_CLEVEL] + (borg_skill[BI_CLEVEL] / 2 * (MAX(1,borg_skill[BI_CLEVEL] / 10)) ));
      	return (borg_attack_aux_spell_bolt(REALM_DEATH,1,3, rad, dam, GF_OLD_DRAIN));

        /* Dispel Good */
        case BF_DEATH_DISPELGOOD:
        rad = MAX_SIGHT + 10;
        dam = (borg_skill[BI_CLEVEL] * 4) / 2;
        return (borg_attack_aux_spell_dispel(REALM_DEATH,1,5, rad, dam, GF_DISP_GOOD));
        ...
        dam = 300;
        return (borg_attack_aux_spell_bolt(REALM_DEATH,2,4, rad, dam, GF_OLD_DRAIN));
        ...
        return (borg_attack_aux_spell_bolt(REALM_DEATH,3,0, rad, dam, GF_DEATH_RAY));

        /* Word of Death */
        case BF_DEATH_WORDOFDEATH:
        rad = MAX_SIGHT + 10;
        dam = (3 * borg_skill[BI_CLEVEL]) /2;
        return (borg_attack_aux_spell_bolt(REALM_DEATH,3,3, rad, dam, GF_OLD_DRAIN));

        /* Evocation */
        case BF_DEATH_EVOCATION:
        dam = borg_skill[BI_CLEVEL] * 2;
        return (borg_attack_aux_spell_bolt(REALM_DEATH,3,4, rad, dam, GF_OLD_DRAIN));

        /* HellFire */
        case BF_DEATH_HELLFIRE:
        dam = 666;
        if (borg_skill[BI_CURHP] < 200) dam = 0;
        return (borg_attack_aux_spell_bolt(REALM_DEATH,3,5, rad, dam, GF_OLD_DRAIN));

        /* Mind-- Neural Blast */
        case BF_MIND_NEURAL:
        dam = 3+((borg_skill[BI_CLEVEL] - 1)/4) * (3+(borg_skill[BI_CLEVEL]/15)) / 2;
		rad = -1;
		...
		{
	        dam = borg_skill[BI_CLEVEL] * ((borg_skill[BI_CLEVEL] - 5)/11);
			rad = 10 + MAX_SIGHT;
		}
        return (borg_attack_aux_mind_bolt(MIND_MIND_WAVE, 18, rad, dam, GF_PSI));

        /* Mind-- Psychic Drain */
        case BF_MIND_PSYCH_DRAIN:
        dam = ((borg_skill[BI_CLEVEL]/2) * 6) / 2;
		rad = (borg_skill[BI_CLEVEL] - 25) / 10;
        return (borg_attack_aux_mind_bolt(MIND_PSYCHIC_DR, 25, rad, dam, GF_PSI_DRAIN));
        ...
        return (borg_attack_aux_mind_bolt(MIND_TELE_WAVE, 28, rad, dam, GF_TELEKINESIS));
...
*/
    ...
    /* Hack -- Dragon Scale Mail can be activated as well */
        ...
        return (borg_attack_aux_racial_bolt(RACE_DRACONIAN, rad, dam, GF_FIRE));
        ...
        return (borg_attack_aux_racial_bolt(RACE_KLACKON, rad, dam, GF_ACID));

		/* Poison bolt */
        case BF_RACIAL_KOBOLD:
        dam = borg_skill[BI_CLEVEL];
        return (borg_attack_aux_racial_bolt(RACE_KOBOLD, rad, dam, GF_POIS));
        ...
        dam = borg_skill[BI_CLEVEL];
        rad = 10 + MAX_RANGE;
        return (borg_attack_aux_racial_bolt(RACE_SPRITE, rad, dam, GF_OLD_SLEEP));
...
static int borg_defend_aux_glyph( int p1)
{
    ...
    if (borg_spell_okay_fail(REALM_LIFE,1,7,fail_allowed)) glyph_spell = TRUE;
...
static int borg_defend_aux_true_warding( int p1)
{
    ...
    for (glyph_x = -1; glyph_x <= 1; glyph_x++)
    {
        for (glyph_y = -1; glyph_y <= 1; glyph_y++)
        {
            ...
            if ((mb_ptr->terrain >= FEAT_CLOSED) && (mb_ptr->terrain <= FEAT_PERM_SOLID))
            {
                glyph_bad++;
            }
...
static int borg_defend_aux_genocide(void)
{
    ...
    if (b_threat_id)
    {
        ...
        if (borg_skill[BI_CURHP] < borg_skill[BI_MAXHP] || borg_skill[BI_CURHP] < 375) b_threat_id = 0;
        ...
        if ((b_num[b_threat_id] * 4)*11/10 >= borg_skill[BI_CURHP]) b_threat_id = 0;
...
static int borg_defend_aux_genocide_hounds(void)
{
    ...
    /* Not if I am weak */
    if (borg_skill[BI_CURHP] < borg_skill[BI_MAXHP] || borg_skill[BI_CURHP] < 350) return (0);
...
static int borg_defend_aux_destruction(void)
{
    ...
    if (borg_skill[BI_AESCAPE] && !borg_skill[BI_ISBLIND] && !borg_skill[BI_ISCONFUSED]) return (0);
...
bool borg_defend(int p1)
{
    ...
    if (borg_goi && borg_goi < (borg_game_ratio *2))
    {
        ...
        if (p > borg_fear_region[c_y/11][c_x/11] ||
            borg_fighting_unique)
        {
            if (borg_spell(REALM_ARCANE,3, 7) ||
...
static int borg_perma_aux_hero(void)
{
    ...
    /* do it! */
    if (borg_spell(REALM_LIFE,7,0) ||
    	borg_spell(REALM_DEATH,7,0))
          return 1;
...
static int borg_perma_aux_glyph(void)
{
    ...
    for (wall_x = -1; wall_x <= 1; wall_x++)
    {
        for (wall_y = -1; wall_y <= 1; wall_y++)
        {
            ...
            if ( /* (mb_ptr->terrain == FEAT_GLYPH) || */
               ((mb_ptr->terrain >= FEAT_MAGMA) && (mb_ptr->terrain <= FEAT_WALL_SOLID)))
            {
                wall_count++;
            }
        }
...
bool borg_check_rest(void)
{
    ...
    for (i = 1; i < borg_kills_nxt; i++)
    {
         ...
         */
        if (d < 13 && (p > avoidance) &&
           avoidance < borg_skill[BI_MAXHP]/2)
           disturb_move = TRUE;
...
bool borg_recover(void)
{
    ...
    if (borg_skill[BI_MAXSP] && borg_skill[BI_CLEVEL] < 25 && borg_skill[BI_CURSP] < borg_skill[BI_MAXSP] &&
        p == 0)
    {
        if (!borg_skill[BI_ISWEAK] && !borg_skill[BI_ISCUT] && !borg_skill[BI_ISHUNGRY] && !borg_skill[BI_ISPOISONED])
        {
...
static bool borg_play_step(int y2, int x2)
{
...
#if 0
    /* Glyph of Warding */
    if (mb_ptr->terrain == FEAT_MINOR_GLYPH ||
    	mb_ptr->terrain == FEAT_GLYPH)
    {
        /* Message */
    ...
    if ((mb_ptr->terrain >= FEAT_DOOR_HEAD + 0x08) && (mb_ptr->terrain <= FEAT_DOOR_TAIL))
    {
        /* Paranoia XXX XXX XXX */
        if (one_in_(100)) return (FALSE);
...
#if 0 /* Need to use fields */
    /* Shops -- Enter */
    if ((mb_ptr->terrain >= FEAT_SHOP_HEAD) && (mb_ptr->terrain <= FEAT_SHOP_TAIL))
    {
        /* Message */
        borg_note(format("# Entering a '%d' shop", (mb_ptr->terrain - FEAT_SHOP_HEAD) + 1));
    ...
    if (borg_skill[BI_MAXCLEVEL] <= 3 && borg_skill[BI_CDEPTH] && !borg_skill[BI_ISSEARCHING] && borg_needs_searching)
    {
        borg_keypress('S');
    }
...
bool borg_flow_stair_more(int why)
{
    ...
    /* dont go down if hungry or low on food, unless fleeing a scary town */
    if (borg_skill[BI_CDEPTH] && (borg_skill[BI_ISWEAK] || borg_skill[BI_ISHUNGRY] || borg_skill[BI_FOOD] < 2))
        return (FALSE);
...
bool borg_flow_kill_corridor(bool viewable)
{
    ...
    for (o_x = -1; o_x <= 1; o_x++)
    {
        for (o_y = -1; o_y <= 1; o_y++)
        {
            ...
            if (mb_ptr->terrain != FEAT_NONE && mb_ptr->terrain < FEAT_MAGMA) continue;
            if (mb_ptr->terrain >= FEAT_PERM_EXTRA && mb_ptr->terrain <= FEAT_PERM_SOLID)
            {
                perma_grids ++;
                continue;
            }
...
bool borg_flow_kill(bool viewable, int nearness)
{
    ...
    for (hall_x = -1; hall_x <= 1; hall_x++)
    {
        for (hall_y = -1; hall_y <= 1; hall_y++)
        {
            ...
            if (/* (mb_ptr->terrain == FEAT_GLYPH) ||
            	(mb_ptr->terrain == FEAT_MINOR_GLYPH) || */
               ((mb_ptr->terrain >= FEAT_MAGMA) && (mb_ptr->terrain <= FEAT_PERM_SOLID)))
            {
                hall_walls++;
            }
...
static bool borg_flow_dark_interesting(int y, int x, int b_stair)
{
    ...
    if ((mb_ptr->terrain >= FEAT_TRAP_TRAPDOOR) && (mb_ptr->terrain <= FEAT_TRAP_SLEEP))
    {
        ...
        if (borg_skill[BI_CDEPTH] == 99 && mb_ptr->terrain == FEAT_TRAP_TRAPDOOR) return (FALSE);
    ...
    for (y = 1; y < AUTO_MAX_Y-1; y++)
    {
        for (x = 1; x < AUTO_MAX_X-1; x++)
        {
            ...
            for (i = 0; i < 4; i++)
            {
                ...
                /* Walls, Doors */
                if (((mb_ptr->terrain >= FEAT_SECRET) && (mb_ptr->terrain <= FEAT_PERM_SOLID)) ||
