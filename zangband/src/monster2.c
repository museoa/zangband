...
void delete_monster_idx(int i)
{
	...
	if (TEST_FLAG(r_ptr->flags, 1, RF1_MULTIPLY)) num_repro--;
...
void compact_monsters(int size)
{
	...
	for (num = 0, cnt = 1; num < size; cnt++)
	{
		...
		for (i = 1; i < m_max; i++)
		{
			...
			if ((TEST_FLAG(r_ptr->flags, 0, RF0_QUESTOR)) && (cnt < 1000)) chance = 100;

			/* Try not to compact Unique Monsters */
			if (TEST_FLAG(r_ptr->flags, 0, RF0_UNIQUE)) chance = 99;
...
void get_mon_num_prep(monster_hook_type monster_hook)
{
	...
	for (i = 0; i < alloc_race_size; i++)
	{
		...
		if ((!monster_hook || (*monster_hook) (entry->index))
			&& (silly_monsters || !(RF_FLAG(r_info[entry->index].flags, 6, SILLY))))
		{
			/* Accept this monster */
...
bool test_monster_square(cave_type *c_ptr, monster_race *r_ptr)
{
	...
	switch (c_ptr->feat)
	{
		...
		{
			if (!(RF_FLAG(r_ptr->flags, 7, WILD_OCEAN))) return (FALSE);
		...
		{
			if (!((RF_FLAG(r_ptr->flags, 6, AQUATIC)) ||
				(RF_FLAG(r_ptr->flags, 6, CAN_FLY)) ||
				(RF_FLAG(r_ptr->flags, 6, CAN_SWIM))))
			{
				return (FALSE);
			}
		...
		{
			if (RF_FLAG(r_ptr->flags, 1, AURA_FIRE)) return (FALSE);
		...
		case FEAT_SHAL_LAVA:
		{
			...
			if (RF_FLAG(r_ptr->flags, 2, IM_FIRE)) return (TRUE);
		
			/* If we are cold - then we can't cross */
			if (RF_FLAG(r_ptr->flags, 2, AURA_COLD)) return (FALSE);
			
			/* If we can't fly, then we can't cross */
			if (!(RF_FLAG(r_ptr->flags, 6, CAN_FLY))) return (FALSE);
			break;
		}
		case FEAT_DEEP_ACID:
		case FEAT_SHAL_ACID:
		{
			...
			if (RF_FLAG(r_ptr->flags, 2, IM_ACID)) return (TRUE);
		...
		case FEAT_SHAL_SWAMP:
		{
			...
			if (RF_FLAG(r_ptr->flags, 2, IM_POIS)) return (TRUE);
	...
	/* Aquatic monster */
	if ((RF_FLAG(r_ptr->flags, 6, AQUATIC)) && !(RF_FLAG(r_ptr->flags, 6, CAN_FLY)))
	{
		return FALSE;
	}
...
static bool test_monster_wild(wild_done_type *w_ptr, monster_race *r_ptr)
{
	...
	if (w_ptr->place)
	{
		...
		if ((!place[w_ptr->place].quest_num) &&
			(RF_FLAG(r_ptr->flags, 7, WILD_TOWN))) return TRUE;
	}
	...
	{
		if (RF_FLAG(r_ptr->flags, 7, WILD_OCEAN)) return TRUE;
	...
	{
		if (RF_FLAG(r_ptr->flags, 7, WILD_SHORE)) return TRUE;
	...
	if (w_ptr->info & WILD_INFO_ACID)
	{
		...
		/* If we can't fly, then we can't cross */
		if (RF_FLAG(r_ptr->flags, 6, CAN_FLY)) return TRUE;
	...
	if (!mon_wild)
	{
		...
		if (RF_FLAG(r_ptr->flags, 7, WILD_GRASS)) return TRUE;
	}
...
s16b get_mon_num(int level)
{
	...
	for (i = 0; i < alloc_race_size; i++)
	{
		...
		if (((TEST_FLAG(r_ptr->flags, 0, RF0_UNIQUE)) || (TEST_FLAG(r_ptr->flags, 2, RF2_UNIQUE_7)))
		...
		if ((TEST_FLAG(r_ptr->flags, 0, RF0_FORCE_DEPTH))
...
void monster_desc(char *desc, const monster_type *m_ptr, int mode, int max)
{
	...
	if (p_ptr->tim.image)
	{
		...
		if (!named)
		{
			...
			while (RF_FLAG(hallu_race->flags, 0, UNIQUE));
	...
	if (!seen || pron)
	{
		...
		if (TEST_FLAG(r_ptr->flags, 0, RF0_FEMALE)) kind = 0x20;
		else if (TEST_FLAG(r_ptr->flags, 0, RF0_MALE)) kind = 0x10;
	...
	else if ((mode & 0x02) && (mode & 0x01))
	{
		...
		if (RF_FLAG(r_ptr->flags, 0, FEMALE)) strnfmt(desc, max, "herself");
		else if (RF_FLAG(r_ptr->flags, 0, MALE)) strnfmt(desc, max, "himself");
		else
	...
	else
	{
		...
		if ((RF_FLAG(r_ptr->flags, 0, UNIQUE)) && !p_ptr->tim.image)
		{
...
void lore_treasure(int m_idx, int num_item, int num_gold)
{
	...
	if (TEST_FLAG(r_ptr->flags, 0, RF0_DROP_GOOD)) r_ptr->r_flags[0] |= (RF0_DROP_GOOD);
	if (TEST_FLAG(r_ptr->flags, 0, RF0_DROP_GREAT)) r_ptr->r_flags[0] |= (RF0_DROP_GREAT);
...
void update_mon(int m_idx, bool full)
{
	...
	if (d <= MAX_SIGHT)
	{
		...
		if (TEST_FLAG(p_ptr->flags, 2, TR2_TELEPATHY))
		{
			...
			if (TEST_FLAG(r_ptr->flags, 1, RF1_EMPTY_MIND))
			{
				/* Memorize flags */
			...
			else if (TEST_FLAG(r_ptr->flags, 1, RF1_WEIRD_MIND))
			{
				...
				if ((m_idx % 10) == 5)
				{
					...
					if (TEST_FLAG(r_ptr->flags, 1, RF1_SMART)) r_ptr->r_flags[1] |=
							(RF1_SMART);
					if (TEST_FLAG(r_ptr->flags, 1, RF1_STUPID)) r_ptr->r_flags[1] |=
							(RF1_STUPID);
				}
			}
			...
			else
			{
				...
				/* Hack -- Memorize mental flags */
				if (TEST_FLAG(r_ptr->flags, 1, RF1_SMART)) r_ptr->r_flags[1] |= (RF1_SMART);
				if (TEST_FLAG(r_ptr->flags, 1, RF1_STUPID)) r_ptr->r_flags[1] |=
						(RF1_STUPID);
			}
		}
		...
		if (in_boundsp(fx, fy))
		{
			...
			if (player_has_los_grid(pc_ptr) && !p_ptr->tim.blind)
			{
				...
				if (d <= p_ptr->see_infra)
				{
					...
					if (TEST_FLAG(r_ptr->flags, 1, RF1_COLD_BLOOD))
					{
						/* Take note */
						do_cold_blood = TRUE;
					}
				...
				if ((player_can_see_grid(pc_ptr))
					|| (TEST_FLAG(r_ptr->flags, 6, RF6_LITE_1 | RF6_LITE_2)))
				{
					/* Handle "invisible" monsters */
					if (TEST_FLAG(r_ptr->flags, 1, RF1_INVISIBLE))
					{
						/* Take note */
						do_invisible = TRUE;
                       ...
                       bool pet)
{
	...
	if (cave_wall_grid(c_ptr) && !(RF_FLAG(r_ptr->flags, 1, PASS_WALL)))
	{
		return (FALSE);
	}
	...
	if ((TEST_FLAG(r_ptr->flags, 0, RF0_FORCE_DEPTH)) && (p_ptr->depth < r_ptr->level)
		&& (!ironman_nightmare || (TEST_FLAG(r_ptr->flags, 0, RF0_QUESTOR))))
	{
		/* Cannot create */
		return (FALSE);
	}
	...
	if (r_ptr->level > p_ptr->depth)
	{
		...
		else
		{
			...
			if (!(TEST_FLAG(r_ptr->flags, 0, RF0_FRIENDS)))
			{
	...
	/* Note the monster */
	else if (TEST_FLAG(r_ptr->flags, 0, RF0_UNIQUE))
	{
	...
	/* Friendly? */
	else if (friendly || (RF_FLAG(r_ptr->flags, 6, FRIENDLY)))
	{
		set_friendly(m_ptr);
	}
	...
	if (RF_FLAG(r_ptr->flags, 0, FORCE_MAXHP))
	{
	...
	if ((RF_FLAG(r_ptr->flags, 0, FORCE_SLEEP)) && !ironman_nightmare)
	{
	...
	/* Hack - are we a mimic? */
	if (RF_FLAG(r_ptr->flags, 0, CHAR_MIMIC))
	{
	...
	/* Hack -- Count the number of "reproducers" */
	if (RF_FLAG(r_ptr->flags, 1, MULTIPLY)) num_repro++;


	/* Hack -- Notice new multi-hued monsters */
	if (RF_FLAG(r_ptr->flags, 0, ATTR_MULTI)) shimmer_monsters = TRUE;
...
static bool place_monster_okay(int r_idx)
{
	...
	if (RF_FLAG(z_ptr->flags, 0, UNIQUE)) return (FALSE);

	/* Paranoia -- Skip identical monsters */
	if (place_monster_idx == r_idx) return (FALSE);

	/* Good vs. evil */
	if (((RF_FLAG(r_ptr->flags, 2, EVIL)) && (RF_FLAG(z_ptr->flags, 2, GOOD)))
		|| ((RF_FLAG(r_ptr->flags, 2, GOOD)) && (RF_FLAG(z_ptr->flags, 2, EVIL))))
	{
		return FALSE;
	}

	/* Hostile vs. non-hostile */
	if ((RF_FLAG(r_ptr->flags, 6, FRIENDLY)) != (z_ptr->flags[6] & RF6_FRIENDLY))
		return FALSE;
                       ...
                       bool friendly, bool pet, bool summon)
{
	...
	if (TEST_FLAG(r_ptr->flags, 0, RF0_ESCORT))
	{
		...
		for (i = 0; i < 50; i++)
		{
			...
			if ((RF_FLAG(r_info[z].flags, 0, FRIENDS))
				|| (RF_FLAG(r_ptr->flags, 0, ESCORTS)))
			{
...
static bool summon_specific_okay(int r_idx)
{
	...
	if (summon_specific_who > 0)
	{
		...
		/* Good vs. evil */
		if (((RF_FLAG(r_ptr->flags, 2, EVIL)) && (RF_FLAG(s_ptr->flags, 2, GOOD)))
	...
	else if (summon_specific_who < 0)
	{
		...
		if (((p_ptr->align < 0) && (RF_FLAG(r_ptr->flags, 2, GOOD)))
			|| ((p_ptr->align > 0) && (RF_FLAG(r_ptr->flags, 2, EVIL))))
		{
			return FALSE;
		}
	}
	...
	switch (summon_specific_type)
	{
		case SUMMON_ANT:
		{
			okay = ((r_ptr->d_char == 'a') && !(RF_FLAG(r_ptr->flags, 0, UNIQUE)));
			break;
		}

		case SUMMON_SPIDER:
		{
			okay = ((r_ptr->d_char == 'S') && !(RF_FLAG(r_ptr->flags, 0, UNIQUE)));
			break;
		}
		...
		case SUMMON_HYDRA:
		{
			okay = ((r_ptr->d_char == 'M') && !(RF_FLAG(r_ptr->flags, 0, UNIQUE)));
			break;
		}
		...
		case SUMMON_DEMON:
		{
			okay = ((RF_FLAG(r_ptr->flags, 2, DEMON))
					&& !(RF_FLAG(r_ptr->flags, 0, UNIQUE)));
			break;
		}
		...
		case SUMMON_UNDEAD:
		{
			okay = ((RF_FLAG(r_ptr->flags, 2, UNDEAD))
					&& !(RF_FLAG(r_ptr->flags, 0, UNIQUE)));
			break;
		}
		...
		case SUMMON_DRAGON:
		{
			okay = ((RF_FLAG(r_ptr->flags, 2, DRAGON))
					&& !(RF_FLAG(r_ptr->flags, 0, UNIQUE)));
			break;
		}
		...
		{
			okay = ((r_ptr->d_char == 'b') && !(RF_FLAG(r_ptr->flags, 0, UNIQUE)));
			break;
		}
		case SUMMON_BIZARRE3:
		{
			okay = ((r_ptr->d_char == 'Q') && !(RF_FLAG(r_ptr->flags, 0, UNIQUE)));
			break;
		}

		case SUMMON_BIZARRE4:
		{
			okay = ((r_ptr->d_char == 'v') && !(RF_FLAG(r_ptr->flags, 0, UNIQUE)));
			break;
		}

		case SUMMON_BIZARRE5:
		{
			okay = ((r_ptr->d_char == '$') && !(RF_FLAG(r_ptr->flags, 0, UNIQUE)));
			break;
		}
		...
		case SUMMON_BIZARRE6:
		{
			okay =
				(((r_ptr->d_char == '!') || (r_ptr->d_char == '?')
				  || (r_ptr->d_char == '=') || (r_ptr->d_char == '$')
				  || (r_ptr->d_char == '|')) && !(RF_FLAG(r_ptr->flags, 0, UNIQUE)));
			break;
		}

		case SUMMON_CYBER:
		{
			okay = ((r_ptr->d_char == 'U') && (RF_FLAG(r_ptr->flags, 3, ROCKET))
					&& !(RF_FLAG(r_ptr->flags, 0, UNIQUE)));
			break;
		}
		...
		case SUMMON_DAWN:
		{
			okay = ((strstr((r_name + r_ptr->name), "the Dawn"))
					&& !(TEST_FLAG(r_ptr->flags, 0, RF0_UNIQUE)));
			break;
		}

		case SUMMON_ANIMAL:
		{
			okay = ((TEST_FLAG(r_ptr->flags, 2, RF2_ANIMAL))
					&& !(TEST_FLAG(r_ptr->flags, 0, RF0_UNIQUE)));
			break;
		}
		...
		case SUMMON_ANIMAL_RANGER:
		{
			okay = ((TEST_FLAG(r_ptr->flags, 2, RF2_ANIMAL))
					...
					&& !(TEST_FLAG(r_ptr->flags, 2, RF2_DRAGON))
					&& !(TEST_FLAG(r_ptr->flags, 2, RF2_EVIL))
					&& !(TEST_FLAG(r_ptr->flags, 2, RF2_UNDEAD))
					&& !(TEST_FLAG(r_ptr->flags, 2, RF2_DEMON)) && !(r_ptr->flags[3]
														   || r_ptr->flags[4]
														   || r_ptr->flags[5])
					&& !(TEST_FLAG(r_ptr->flags, 0, RF0_UNIQUE)));
			break;
		...
		{
			okay = ((r_ptr->d_char == 'D') && !(TEST_FLAG(r_ptr->flags, 0, RF0_UNIQUE)));
			break;
		}

		case SUMMON_NO_UNIQUES:
		{
			okay = (!(TEST_FLAG(r_ptr->flags, 0, RF0_UNIQUE)));
			break;
		}
...
void update_smart_learn(int m_idx, int what)
{
	...
	if (TEST_FLAG(r_ptr->flags, 1, RF1_STUPID)) return;

	/* Not intelligent, only learn sometimes */
	if (!(TEST_FLAG(r_ptr->flags, 1, RF1_SMART)) && (randint0(100) < 50)) return;
	...
	switch (what)
	{
		case DRS_ACID:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_ACID)) m_ptr->smart |= (SM_RES_ACID);
			if (p_ptr->tim.oppose_acid) m_ptr->smart |= (SM_OPP_ACID);
			if (TEST_FLAG(p_ptr->flags, 1, TR1_IM_ACID)) m_ptr->smart |= (SM_IMM_ACID);
			break;
		}
		case DRS_ELEC:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_ELEC)) m_ptr->smart |= (SM_RES_ELEC);
			if (p_ptr->tim.oppose_elec) m_ptr->smart |= (SM_OPP_ELEC);
			if (TEST_FLAG(p_ptr->flags, 1, TR1_IM_ELEC)) m_ptr->smart |= (SM_IMM_ELEC);
			break;
		}
		case DRS_FIRE:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_FIRE)) m_ptr->smart |= (SM_RES_FIRE);
			if (p_ptr->tim.oppose_fire) m_ptr->smart |= (SM_OPP_FIRE);
			if (TEST_FLAG(p_ptr->flags, 1, TR1_IM_FIRE)) m_ptr->smart |= (SM_IMM_FIRE);
			break;
		}
		case DRS_COLD:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_COLD)) m_ptr->smart |= (SM_RES_COLD);
			if (p_ptr->tim.oppose_cold) m_ptr->smart |= (SM_OPP_COLD);
			if (TEST_FLAG(p_ptr->flags, 1, TR1_IM_COLD)) m_ptr->smart |= (SM_IMM_COLD);
			break;
		}
		case DRS_POIS:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_POIS)) m_ptr->smart |= (SM_RES_POIS);
			if (p_ptr->tim.oppose_pois) m_ptr->smart |= (SM_OPP_POIS);
			break;
		}
		case DRS_NETH:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_NETHER)) m_ptr->smart |= (SM_RES_NETH);
			break;
		}
		case DRS_LITE:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_LITE)) m_ptr->smart |= (SM_RES_LITE);
			break;
		}
		case DRS_DARK:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_DARK)) m_ptr->smart |= (SM_RES_DARK);
			break;
		}
		case DRS_FEAR:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_FEAR)) m_ptr->smart |= (SM_RES_FEAR);
			break;
		}
		case DRS_CONF:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_CONF)) m_ptr->smart |= (SM_RES_CONF);
			break;
		}
		case DRS_CHAOS:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_CHAOS)) m_ptr->smart |= (SM_RES_CHAOS);
			break;
		}
		case DRS_DISEN:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_DISEN)) m_ptr->smart |= (SM_RES_DISEN);
			break;
		}
		case DRS_BLIND:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_BLIND)) m_ptr->smart |= (SM_RES_BLIND);
			break;
		}
		case DRS_NEXUS:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_NEXUS)) m_ptr->smart |= (SM_RES_NEXUS);
			break;
		}
		case DRS_SOUND:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_SOUND)) m_ptr->smart |= (SM_RES_SOUND);
			break;
		}
		case DRS_SHARD:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_RES_SHARDS)) m_ptr->smart |= (SM_RES_SHARD);
			break;
		}
		case DRS_FREE:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_FREE_ACT)) m_ptr->smart |= (SM_IMM_FREE);
			break;
		}
		case DRS_MANA:
		{
		...
		case DRS_REFLECT:
		{
			if (TEST_FLAG(p_ptr->flags, 1, TR1_REFLECT)) m_ptr->smart |= (SM_IMM_REFLECT);
			break;
		}
	}
}
