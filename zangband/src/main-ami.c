/*
	...
	Todo:
		...
		Finish window backgrounds; allow proper tiling. Sort of started this.
		Remove window scroller code, as it's ugly and stupid? Does anyone
			actually /use/ this?
		Allow menu definitions for term windows too. Easy enough.
		I can't think of anything else that needs doing. Great! :)

	Changes:
		Graphics should load faster in most cases now.
		Looks in AngSound: for sounds as well as xtra/sound
		Added 'Toggle Borders' option in window menus, as well as
			corresponding 'noborders' option in settings.prf.
		Fixed broken options that turned Graphics on/off/etc.
		Added some extra code that /might/ improve stability when
			opening/closing term windows on certain systems?
...
void amiga_makepath( char *name )
{
	...
	do
	{
		...
		AddPart(temp, "edit/r_info.txt", 500);
		if (f = fopen(temp,"r"))
			break;
		strcpy(temp, temp2);
		AddPart(temp, "data/r_info.raw", 500);
		if (f = fopen(temp,"r"))
			break;
		pass++;
	} while (pass < 2);
