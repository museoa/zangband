...
#ifdef SIGCHLD
	(void)signal(SIGCHLD, handle_signal_abort);
#endif
...
#ifdef SECURE
bool validate_path(cptr buf)
{
	/* No dodgy relative paths */
	if (strstr(buf,"..")) return (1);

	/* Can be in the highscore directory */
	if (strstr(buf, ANGBAND_DIR_APEX) == buf) return (0);

	/* Can be in the bone directory */
	if (strstr(buf, ANGBAND_DIR_BONE) == buf) return (0);

	/* Can be in the data directory */
	if (strstr(buf, ANGBAND_DIR_DATA) == buf) return (0);

	/* Can be in the info directory */
	if (strstr(buf, ANGBAND_DIR_INFO) == buf) return (0);

	/* Can be in the save directory */
	if (strstr(buf, ANGBAND_DIR_SAVE) == buf) return (0);

	/*
	 * Failure - not authorized for write or read with current permissions
	 * Note that files in other directories should be opened with group_games = FALSE.
	 */
	return (1);
}
...
void init_setuid(void)
{
	...
	/* Launch secure child or just drop permissions */
	init_secure();
