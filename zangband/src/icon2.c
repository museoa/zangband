	...
	Tcl_Obj *CONST objv[])
{
	...
	if (objC == 3)
	{
		...
		{
			case 0: /* -background */
				Tcl_SetIntObj(resultPtr, g_background[f_idx]);
				break;
				
			case 1: /* -light */
				Tcl_SetStringObj(resultPtr,
					(char *) keyword_feat_lite[g_feat_lite[f_idx]], -1);
				break;
...
static int ValidateSprite(Tcl_Interp *interp, int spriteIndex)
{
	...
	if ((spriteIndex < 0) ||(spriteIndex >= g_sprite_count))
	{
		...
		/* Set the error */
		Tcl_SetStringObj(resultPtr, format("bad sprite \"%d\": "
...
static int ValidateFrame(Tcl_Interp *interp, int spriteIndex, int frameIndex)
{
	...
	if ((frameIndex < 0) || (frameIndex >= spritePtr->count))
	{
		...
		/* Set the error */
		Tcl_SetStringObj(resultPtr,
			format("bad frame \"%d\" on sprite \"%d\": "
...
/*	Tcl_Obj *CONST *objV = objv + infoCmd->depth; */
	...
	spritePtr->icon = (IconSpec *) Array_New(1, sizeof(IconSpec));

	/* Return the index of the new sprite */
	Tcl_SetIntObj(resultPtr, g_sprite_count - 1);
	...
	Tcl_Obj *CONST objv[])
{
	...
	if (objC == 2)
	{
		...
		Tcl_SetIntObj(resultPtr, spritePtr->count);
	...
	Tcl_SetIntObj(resultPtr, g_sprite_count);
	...
	Tcl_Obj *CONST objv[])
{
	...
	if (objC == 3)
	{
		...
		{
			case 0: /* -delay */
				Tcl_SetIntObj(resultPtr, spritePtr->speed);
				break;

			case 1: /* -reverse */
				Tcl_SetBooleanObj(resultPtr, spritePtr->reverse);
				break;
		}
