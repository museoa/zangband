...
static void borg_notice_aux2(void)
{
    ...
    for (i = 0; i < INVEN_PACK; i++)
    {
        ...
        switch (item->tval)
        {
            ...
            {
                if (borg_spell_okay_fail(REALM_LIFE, 7, 3, 40) && item->iqty >= 5)
                {
                    if (item->to_h < 10)
                    {
    ...
    if (borg_spell_legal_fail(REALM_ARCANE, 3, 6, 40) ||
        ...
        (borg_skill[BI_CDEPTH] == 100 && (borg_spell_legal(REALM_LIFE, 3, 6) ||
         borg_spell_legal(REALM_SORCERY, 2, 7) || borg_spell_legal(REALM_TRUMP, 1, 6))))
...
static void borg_notice_home_aux2(borg_item *in_item, bool no_items)
{
   ...
   /* apw, again.  Artifact activation included here */
    ...
    /* Handle "satisfy hunger" -> infinite food */
    if (borg_spell_legal_fail(REALM_SORCERY, 2, 0, 10) || borg_spell_legal_fail(REALM_LIFE, 0, 7, 10) ||
        borg_spell_legal_fail(REALM_ARCANE, 2, 7, 10) || borg_spell_legal_fail(REALM_NATURE, 0, 3, 10))
    {
        num_food += 1000;
    }
...
extern s32b borg_power_aux2(void)
{
    ...
    if (amt_digger > 1 || borg_items[INVEN_WIELD].tval == TV_DIGGING) value -= 100000L;
...
static s32b borg_power_aux3(void)
{
        ...
        (my_stat_ind[A_INT] <= 37 ))
    {
        ...
        if (borg_class == CLASS_MAGE)
        {
            ...
            if (borg_spell_legal(REALM_SORCERY, 3, 7) ||
                borg_spell_legal(REALM_LIFE, 3, 7)) value += my_stat_ind[A_INT] * 35000L;
        ...
        (my_stat_ind[A_WIS] <= 37 ))
    {
        ...
        if (borg_class == CLASS_PRIEST)
        {
            ...
            if (borg_spell_legal(REALM_LIFE,2, 6)) value += my_stat_ind[A_WIS] * 35000L;
    ...
    if ((borg_class == CLASS_WARRIOR ||
         borg_class == CLASS_CHAOS_WARRIOR) && borg_skill[BI_RFEAR]) value +=2000L;
...
cptr borg_restock(int depth)
{
    ...
    /* Must have "cure" */
    if ((borg_skill[BI_MAXCLEVEL] < 30) && borg_skill[BI_ACSW] + borg_skill[BI_ACCW] < 6) return ("rs cure");
...
static cptr borg_prepared_aux2(int depth)
{
    ...
    if ((borg_skill[BI_MAXCLEVEL] < 30) && borg_skill[BI_ACSW] + borg_skill[BI_ACCW] < 5) return ("5 cures");
...
/*** Essential Items for Level 100 ***/
    ...
    if (!borg_skill[BI_KING])
    {
        if ((borg_skill[BI_MAXSP] > 100) && (borg_has[266] < 15)) return ("15ResMana");
