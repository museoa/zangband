...
extern s32b borg_power_aux2(void)
{
	...
	/* Hack -- most gloves hurt magic for spell-casters */
	if ((borg_class == CLASS_RANGER || borg_class == CLASS_ROGUE))
	{
		item = &borg_items[INVEN_HANDS];
	...
	if (item->flags3 & TR3_REGEN && borg_skill[BI_ISWEAK] &&
		 ...
		 (!borg_skill[BI_IFIRE] && weapon_swap_immune_fire)))
	{
		...
		{
			if (!borg_skill[BI_ICOLD] &&
				weapon_swap_immune_cold) value += 90000L;
			if (!borg_skill[BI_IFIRE] &&
				weapon_swap_immune_fire) value += 90000L;
		}
	}
	...
	for (iEntry = 0; iEntry < nEntrys; iEntry++)
	{
		...
		if (Req_item->item == -1)
		{
			if (!borg_calc_formula(formula[Req_item->number]))
			{
				 ...
				 */
				if (borg_plays_risky &&
					strstr(borg_prt_formula(formula[Req_item->number]),
						   "CLEVEL"))
				{
					continue;
				}
				...
				if (borg_slow_return)
					...
					sprintf(ret_string, "Formula failed FORMULA%03d",
							Req_item->number);
				return ret_string;
			}
			continue;
