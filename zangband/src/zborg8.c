...
static void borg_think_home_sell_aux2_slow(  int n, int start_i )
{
    ...
    if ((n != 0) && !borg_shops[BORG_HOME].ware[n].iqty && !borg_shops[BORG_HOME].ware[n-1].iqty)
        return;
    ...
    if (!borg_items[INVEN_PACK-1].iqty)
    {
        for (i = 0; i < STORE_INVEN_MAX; i++)
        {
                ...
                (borg_shops[BORG_HOME].ware[i].iqty))
            {
                ...
                sold_item_pval == item2->pval && sold_item_store == BORG_HOME) return (FALSE);
...
static bool borg_think_home_buy_aux(void)
{
    ...
    for (n = 0; n < STORE_INVEN_MAX; n++)
    {
        ...
        /* Save shop item */
        COPY(&safe_shops[BORG_HOME].ware[n], &borg_shops[BORG_HOME].ware[n], borg_item);
        ...
        if (slot >= 0)
        {
            ...
            borg_note(format("   Against Item %s   (borg_power %ld)",safe_items[slot].desc, my_power));
                ...
                if (!borg_items[INVEN_RIGHT].cursed)
                {
                    ...
                    /* Move new item into equipment */
                    COPY(&borg_items[INVEN_RIGHT], &safe_shops[BORG_HOME].ware[n], borg_item);
        ...
        /* Restore shop item */
        COPY(&borg_shops[BORG_HOME].ware[n], &safe_shops[BORG_HOME].ware[n], borg_item);
...
static bool borg_think_home_grab_aux(void)
{
    ...
    for (n = 0; n < STORE_INVEN_MAX; n++)
    {
        ...
        /* skip stuff if we sold bought it */
        if (sold_item_tval == item->tval && sold_item_sval == item->sval &&
            sold_item_pval == item->pval && sold_item_store == BORG_HOME) continue;
...
bool borg_think_dungeon(void)
{
    ...
    if (!borg_skill[BI_CDEPTH] && borg_money_scum_amount != 0 && borg_food_onsale == 0)
    {
    ...
    {
        borg_note(format("# Pressing 'space' to catch up and get in sync (%d).", borg_respawning));
        borg_keypress(' ');
        borg_keypress(ESCAPE);
        borg_keypress(ESCAPE);
        borg_respawning --;
    ...
    if (borg_skill[BI_CUR_LITE] == 0 || borg_skill[BI_ISCUT] || borg_skill[BI_ISPOISONED] || borg_skill[BI_ISWEAK])
    {
		...
		if (borg_grids[c_y][c_x].feat == FEAT_MORE && borg_flow_town_exit(GOAL_TOWN)) return (TRUE);
    ...
    if (borg_skill[BI_CLEVEL] < 10 && borg_skill[BI_MAXSP] && borg_skill[BI_CURSP] == 0)
    {
        if (borg_skill[BI_CDEPTH])
        {
            int i, y, x;
