...
static int borg_danger_aux1(int r_idx)
{
	...
	if ((borg_prot_from_evil) &&
		(RF_FLAG(r_ptr->flags, 2, EVIL)) && (bp_ptr->lev >= r_ptr->level))
	{
		pfe = 1;
	}
	...
	for (q = 0; q < num; q++)
	{
		...
		switch (spell[q])
		{
			...
			case 160 + 16:
			{
				...
				if (RF_FLAG(r_info->flags, 0, UNIQUE)) p = p * 75 / 100;
...
int borg_danger_aux(int x, int y, int c, int i, bool average)
{
	...
	if (RF_FLAG(r_ptr->flags, 0, NEVER_BLOW))
	{
		v1 = 0;
	}

	/* No movement for some monsters */
	if ((RF_FLAG(r_ptr->flags, 0, NEVER_MOVE)) && (d > 1))
	{
		v1 = 0;
	}
	...
	if ((RF_FLAG(r_ptr->flags, 1, MULTIPLY)) && (bp_ptr->lev < 20))
	{							/* extra 50% */
		v1 = v1 + (v1 * 15 / 10);
	}

	/* Friends yeild some trouble when I am weak */
	if ((RF_FLAG(r_ptr->flags, 0, FRIENDS) || RF_FLAG(r_ptr->flags, 0, ESCORTS)) &&
		(bp_ptr->lev < 20))
	{
		if (bp_ptr->lev < 15)
		{
	...
	if (v1)
	{
		...
		if (c > 1)
		{
			...
			if (TEST_FLAG(r_ptr->flags, 0, RF0_RAND_25 | RF0_RAND_50)) r -= (r / 4);
		}
		/* Total danger */
		v1 = v1 * r / 10;
