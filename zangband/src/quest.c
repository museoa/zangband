...
#if 0
/*
 * Hack - try to pluralize a monster name.
 *
 * Known cases we won't handle correctly:
 *   Creeping * coins (shouldn't change)
 *
 * We ignore some special plurals, like "Mumak" -> "Mumakil" or
 * "Seraph" -> "Seraphim", when the current result also sounds okay.
 *
 * Commented out because it more or less duplicates plural_aux().
 */
static cptr pluralize_name(cptr name)
{
    char *p;
    static char buf[80];
    char tail[80];
    ...
    /* Handle some cases where we pluralize with "es" */
    if ((len >= 1 && streq(buf - 1, "s")) ||
        (len >= 1 && streq(buf - 1, "x")) ||
        (len >= 2 && streq(buf - 2, "sh")) ||
        (len >= 2 && streq(buf - 2, "ch")))
    {
        strcpy(buf + len, "es");
        return (buf);
    }
    ...
    /* Handle "y" -> "ies" */
    else if (len >= 2 && streq(buf - 2, "ey"))
    {
        strcpy(buf + len - 2, "ies");
    }
    else if (len >= 1 && streq(buf - 1, "y"))
    {
        strcpy(buf + len - 1, "ies");
    }

    /* Handle "man" -> "men", except "human", "shaman", "Pukelman" */
    else if (len >= 3 && streq(buf - 3, "man") &&
             !(len >= 5 && streq(buf - 5, "human")) &&
             !(len >= 6 && streq(buf - 6, "shaman")) &&
             !(len >= 6 && streq(buf - 6, "elman")))
    {
        strcpy(buf + len - 3, "men");
    }

    /* Handle "ouse" -> "ice" (louse, mouse) */
    else if (len >= 4 && streq(buf - 4, "ouse"))
    {
        strcpy(buf + len - 4, "ice");
    }

    /* Handle "f" -> "ves" (elf, wolf, thief) except "griff" */
    else if (len >= 1 && streq(buf - 1, "f") &&
             !(len >= 2 && streq(buf - 2, "ff")))
    {
        strcpy(buf + len - 1, "ves");
    }

    /* Handle "mage" -> "magi" */
    else if (len >= 4 && streq(buf - 4, "mage"))
    {
        strcpy(buf + len - 4, "magi");
    }
